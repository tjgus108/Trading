...
_Last updated: 2026-05-22 (Cycle 193)_
...
- **리스크**: **Kelly(rolling win_rate 동적추정+quarter-cap+레짐+MDD+스무딩+stress)** + BayesianKelly(β prior+fractional0.33) + VaR(CF+backtest) + DrawdownMonitor(4단계+cooldown) + VolTargeting(simple+EWMA) + CircuitBreaker(일일제한+급속하락감지+직렬화검증)
...
| 카테고리 | 상태 | 주요 변경 |
|---------|------|----------|
| C (데이터) | ✅ | WebSocket ConnectionHealthMonitor (stale 감지, 재연결 이력, health summary) |
| B (리스크) | ✅ | KellySizer rolling win_rate 동적 추정 (record_trade + compute_dynamic) |
| F (리서치) | ✅ | 온체인: Exchange Inflow 2σ→1주 하락 72%, 최우선 신호 3개 도출 |
