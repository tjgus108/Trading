...
_Last updated: 2026-04-20 (Cycle 167)_
...
- **테스트**: 7,400+ passed (risk 174, feed 93, exchange 98, trainer 63+, FR/OI 24+25, kelly 77, feature_builder 29, drift 53, backtest 48 포함)
...
- **라이브**: live_paper_trader 90% 준비 (max_loss_pct+PSI통합+XGBoost+앙상블, 37개 테스트)

## 최근 작업 (Cycle 167)
| 카테고리 | 상태 | 주요 변경 |
|---------|------|----------|
| B (리스크) | ✅ | Kelly 레짐 스무딩(EMA) + Cornish-Fisher VaR, 테스트 4개 |
| D (ML) | ✅ | MultiWindowEnsemble 30/60/90일 stacking, 테스트 9개 |
| F (리서치) | ✅ | 트레이딩봇 73% 실패 원인 + Stacking 실증 + Paper→Live 갭 분석 |

## 완료된 대응 (Cycle 140~167)
...
- ✅ live_paper_trader max_loss_pct 구현 + min_accuracy 0.55
...
- ✅ MultiWindowEnsemble: 30/60/90일 다시간 stacking + softmax 동적 가중치

## 주요 리스크/이슈
- ⚠️ 실데이터 PASS 전략 0개 — ML 경로가 유일한 희망
- 다음 우선: live_paper_trader 7일 운영, Telegram 알림, Health Check 루프
