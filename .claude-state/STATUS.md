...
_Last updated: 2026-05-31 (Cycle 252)_
...
- **테스트**: 8,356+ passed (Cycle 252 +5: validate_ohlcv 5)
...
- **백테스트**: MC permutation test block sign randomization 버그 수정 + --regime-weights CLI + perturbation_check(ROBUST/MODERATE/FRAGILE) + --perturbation-check CLI + mc_min_trades/mc_block_size 파라미터 + --use-quality-data
...
| 카테고리 | 상태 | 주요 변경 |
|---------|------|----------|
| E (실행) | ✅ | validate_ohlcv() 데이터 검증 헬퍼 (중복/갭/OHLC/음수볼륨, 5 tests) |
| A (품질) | ✅ | DSR → BundleOOSResult 통합 (dsr_pvalue, is_sharpe_significant) |
| F (리서치) | ✅ | 레짐 감지 실패 패턴, 데이터 아키텍처(Freqtrade 구조), SQLite 중복 사례 |
