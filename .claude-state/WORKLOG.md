...
## [2026-05-28] Cycle 235 — A(품질) + C(데이터) + SIM + F(리서치)

**[A] 품질 — MC Permutation Test 버그 수정 (근본 원인)**:
- `src/backtest/engine.py`: `_mc_permutation_test` 단위 불일치 버그 수정
  - **버그**: equity-curve Sharpe(candle-level)를 임계값으로, trade PnL로 permuted Sharpe 계산
    - 111 trades → perm_sharpe ~ N(0, sqrt(8760/111)) ≈ N(0, 8.9)
    - P(Z ≥ 4.22/8.9) ≈ 0.317 → 관측된 mc_p 0.31~0.34 정확히 일치
  - **수정**: permuted Sharpe와 trade-based original Sharpe를 동일 계산식으로 비교
    - `trade_sharpe = arr.mean() / arr.std() * sqrt(8760)` (같은 PnL 배열)
    - 부산물: 전략 PASS 0/22 → 7(BTC), 11(ETH), 4(SOL)

**[C] 데이터 — BlockBootstrap block_size 36→72 확대**:
- `scripts/paper_simulation.py`: BLOCK_BOOTSTRAP_BLOCK_SIZE 기본값 36→72
  - 36봉(1.5일) → 72봉(3일) 블록으로 추세 성분 보존 강화
  - 환경변수 PAPER_SIM_BLOCK_SIZE로 오버라이드 가능

**[F] 리서치 — GBM 합성 vs Block Bootstrap vs 실거래소 괴리**:
- Block Bootstrap 72봉: 합성 데이터에서도 IS Sharpe 음수 비율 감소 확인
- MC test p-value 범위 변화: 0.3+ → 0.06~0.13 (정상 범위)
- 실데이터 PASS 기준은 여전히 SSL 인터셉션으로 검증 불가 → 합성 결과 방향성만 참고

**SIM 결과 (Cycle 235)**:
### Paper (Walk-Forward 1h봉, block=72)
- BTC: **7/22 PASS** (0→7 개선)
  - PASS: price_action_momentum(3/4,Sh5.21), momentum_quality(3/4,Sh3.76), cmf(2/4,Sh3.21), volume_breakout(2/4,Sh3.77), order_flow_imbalance_v2(2/4,Sh3.17), htf_ema(2/4), positional_scaling(3/4)
- ETH: **11/22 PASS** (0→11 개선)
  - PASS: supertrend_multi(4/4,Sh5.54), cmf(3/4,Sh6.23), price_action_momentum(3/4), volatility_cluster(3/4), volume_breakout(3/4), order_flow_imbalance_v2(3/4), momentum_quality(3/4), roc_ma_cross(2/4), narrow_range(2/4), htf_ema(2/4), value_area(2/4)
- SOL: **4/22 PASS** (0→4 개선)
  - PASS: cmf(3/4,Sh4.37), momentum_quality(3/4,Sh4.34), order_flow_imbalance_v2(3/4,Sh3.48), volatility_cluster(2/4)
- 크로스-심볼 PASS (3종 모두): 없음 (ETH 데이터 성질 상이)
- 크로스-심볼 PASS (2종 이상): cmf, momentum_quality, order_flow_imbalance_v2, volume_breakout, htf_ema

### OOS Bundle (4h봉, 합성 GBM)
- **0/5 PASS** — 4h GBM은 여전히 IS Sharpe 음수 (SSL 차단으로 실데이터 불가)
- OOS Sharpe std: 3.4~6.4 (high) — 4h에서 더 극단적 변동
- elder_impulse fold 1: OOS Sharpe 3.79 (유일한 PASS fold)

**테스트**: 8,127 passed (회귀 없음)
...
- `BinanceWebSocketFeed.__init__` 시그니처: `stale_timeout: float = 300.0` → `Optional[float] = None`
- 조건 `stale_timeout > 0` → `stale_timeout is not None` (0 포함 명시적 값 저장)
- 기존 실패 테스트 `test_feed_stale_timeout_param` 수정 → PASS

**[B] 리스크 — KellySizer 레짐별 동적 fraction:**
- `src/risk/kelly_sizer.py`: `_REGIME_FRACTION` 절대값 딕셔너리 추가
  - TREND_UP/BULL → 25% (Quarter-Kelly 표준), HIGH_VOL/CRISIS → 10%
  - TREND_DOWN/BEAR → 15%, RANGING → 20%
- `get_dynamic_fraction(regime)`: 레짐별 절대 fraction 반환
- `update_fraction_for_regime(regime)`: self.fraction 동적 갱신 + 반환
- 테스트: 11개 추가 (TestKellySizerDynamicFraction)

**[D] ML — VPIN 피처 추가 + walk_forward 안정성 개선:**
- `src/ml/features.py`: `vpin_50` 피처 추가 (~12줄)
  - OHLCV만으로 계산: close>open→BUY frac, close<open→SELL frac
  - 50봉 롤링, [0,1] 범위, len<10이면 스킵
  - 출처: Easley et al. (2012) VPIN 플래시 크래시 선행 지표 연구
- `src/backtest/walk_forward.py`: 파라미터 선택 기준 개선
  - 기존: 평균 OOS Sharpe 최대화 → 취약: 고분산 파라미터 선택 가능
  - 신규: Sharpe IC = avg - 0.5 * std (안정성 가중 선택)
- 피처 수 관련 테스트 7개 업데이트 (base 14→15, FR/OI 포함 시 +1 각각)
- 테스트: 5개 신규 (TestFeatureBuilderVPIN)

**[SIM] 시뮬레이션 결과:**
- Paper (Walk-Forward 1h봉): 0/22 PASS, 평균 수익률 +13.22%
  - Top: price_action_momentum (Sharpe 3.81, +49%), momentum_quality (Sharpe 3.91, +38%)
  - 실패 원인: 일관성 0/4 윈도우 (GBM 합성 한계)
- OOS Bundle (4h봉): 0/5 PASS
  - OOS Sharpe std 3.4~6.4 >> 기준 1.5 (불안정)
  - narrow_range 최선 (3/9 fold PASS): IS 음수 대부분
  - 실거래소 데이터 검증 필요

**[F] 리서치 — Sharpe IC + VPIN 성공 사례:**
- Sharpe Information Criterion: 파라미터 선택 시 avg - λ*std 최소화
  - λ=0.5 권장: 과적합 방지 vs 성과 trade-off 균형
  - Walk-Forward에 즉시 적용 (코드 개선 완료)
- VPIN 실거래 성공 사례: Renaissance Tech 등 HFT에서 사용
  - 플래시 크래시(2010) 30분 전 VPIN 급등 감지
  - ML 피처로 추가 시 OFI + VPIN 상관성 분석 필요
- OOS Sharpe std 원인: 레짐 이질성 (is/oos 레짐 불일치) + 짧은 fold 길이

**테스트:** 8101 passed, 23 skipped (이전 7955 → +146)
...
- `src/risk/kelly_sizer.py`: Cornish-Fisher 안전 클리핑 추가
  - 극단 skew/kurtosis(|skew|>5, kurt>50)에서 z_cf 발산 방지
  - `skew = clip(-5, 5)`, `excess_kurt = clip(-2, 50)` 적용
- `src/risk/kelly_sizer.py`: `compute_from_trades(trades: List[float], ...)` 메서드 추가
  - 최근 거래 수익률 리스트 → win_rate/avg_win/avg_loss 자동 추출 후 compute() 호출
  - 소표본 Bayesian shrinkage (n < min_trades=10 시 0.5 방향 수축)
  - 빈 리스트/모두손실/NaN/inf 엣지 케이스 처리
  - `MIN_TRADES_FOR_KELLY = 10` 클래스 상수 추가
- vol_targeting.py / circuit_breaker.py: 이미 완전 구현됨, 변경 없음
- 테스트: 70 passed (kelly integration + regime edge cases)

**[D] ML — WalkForwardTrainer StandardScaler 옵션:**
- `src/ml/trainer.py`: `use_scaler: bool = False` 파라미터 추가 (기존 호환성 유지)
  - use_scaler=True: train set으로 fit_transform, val/cal/test에 transform만 (look-ahead 금지)
  - pkl 저장 시 scaler 포함 (`"scaler": self._scaler`)
  - StandardScaler 미설치 시 자동 비활성화 fallback
- drift_detector.py: delta=0.05, min_window=32 — 이미 적정, 변경 없음
- 테스트: 41 passed (adwin_drift), 53 passed (trainer)

**[SIM] Bundle OOS (4h, BTC/USDT, 합성 GBM):**
- 0/5 PASS — 전 전략 FAIL (cmf, elder_impulse, wick_reversal, narrow_range, value_area)
...
- OOS Sharpe std >1.5 = 파라미터 불안정성 지속 (실전 데이터 검증 필수)
- value_area: 저거래(2-7건/fold) 구조적 문제 → 파라미터 완화 또는 테스트 제외 검토

---

## [2026-05-27] Cycle 225 — A(품질) + B(리스크) + SIM + F(리서치)

**[B] 리스크 — DrawdownMonitor + WalkForwardOptimizer:**
- DrawdownMonitor weekly/monthly 리셋 → orchestrator 연동 완료
- WalkForwardOptimizer plateau_score 추가 (최적 파라미터 ±10% 안정성 검증)

**[SIM] Paper Simulation (BTC 단일, 합성 BlockBootstrap):**
- 0/22 PASS — avg_sharpe=-2.73, avg_pf=0.54
- value_area va_mult=0.6 (0.7→0.6): avg_trades 16→18.2 (+14%)
- 상위전략: volatility_cluster(76.1), price_action_momentum(73.3), momentum_quality(69.4)
- FAIL 원인: mc_p_value > 0.05 (39.6%), profit_factor < 1.5 (32.2%)
...
- `scripts/quality_audit.py`: 모듈 레벨 `logging.setLevel(CRITICAL)` → `run_audit()` 내부로 이동
  - test_block_bootstrap.py 임포트 시 전역 로깅 억제 사이드 이펙트 제거
  - `test_twap_drawdown_logging` 2건 flaky 해소 (전체 suite에서 재현 가능하게)
- `src/risk/manager.py`: `FullCircuitBreakerAdapter` 추가
  - `circuit_breaker.CircuitBreaker`(풀버전)를 `RiskManager` 레거시 인터페이스로 어댑트
  - `peak_balance`, `daily_start_balance` 자동 추적, `reset_daily()` 지원
  - 테스트 3개 신규 추가 (no_trigger, drawdown_trigger, risk_manager 통합)
- `tests/test_risk_manager.py`에 `FullCircuitBreakerAdapter` 테스트 3건 추가

**[C] 데이터 — ConnectionHealthMonitor 개선:**
- `src/data/websocket_feed.py`: `ConnectionHealthMonitor` 개선
  - `_reconnection_history`: list 슬라이스 → `deque(maxlen=10)` 교체
  - `reconnection_rate(window_seconds)` 추가: 기간 내 재연결 횟수 반환
  - `is_flapping(window_seconds, threshold)` 추가: 5분 내 3회+ → 불안정 판정
  - `get_health_summary()`: `is_flapping`, `reconnection_rate_1h` 필드 추가
- 기존 39개 WebSocket 테스트 전체 PASS

**[SIM] 시뮬레이션 결과 (합성 데이터, SSL 차단으로 실거래소 불가):**
- Paper Sim (1h Walk-Forward): 22전략 전부 FAIL (Consistency 0/4)
  - 최우선: `price_action_momentum` (Sharpe 6.68, PF 1.78, Trades 167, MDD 13.4%)
  - `momentum_quality` (Sharpe 6.98, PF 2.03, Trades 116, MDD 8.3%) 2위
- Bundle OOS (4h): 5전략 전부 FAIL, OOS Sharpe std > 1.5 초과가 주 원인
  - `value_area` 최우선 (Score 79.6, 4/9 fold PASS, OOS Sharpe std 4.96)
  - narrow_range: fold 4,6,7 PASS (3/9) — 일부 윈도우에서 작동
- 코드 개선: `walk_forward.py` WalkForwardValidator.validate()에 `fail_reasons` 필드 추가

**[F] 리서치 인사이트:**
- circuit_breaker.py → manager.py 통합 완료 (FullCircuitBreakerAdapter로)
- 합성데이터 한계: OOS Sharpe std 평균 ~4.5 → 실거래소 데이터에서만 정확한 검증 가능
- `value_area` 저거래 fold (2 trades): signal 조건 추가 완화 검토 대상
- RegimeGuardedStrategy: 다음 레짐 사이클(B+D)에서 구현 권장

---

## [2026-05-27] Cycle 220 — D(ML) + E(실행) + SIM + F(리서치)

**[D] ML — DualGateADWINMonitor 재학습 트리거 + 모델 헬스:**
- `src/ml/drift_detector.py`: `should_retrain_by_ewma()` 추가
  - ewma_early_warning 연속 N회(warning_threshold=5) 발생 시 True 반환
  - _consecutive_warnings 카운터, reset/hard_reset 시 0 초기화
- `src/ml/drift_detector.py`: `get_model_health()` 추가
  - accuracy, ewma_accuracy, ewma_trend(improving/degrading/stable/unknown), drift_detected
  - 연속 경고 횟수 포함
- 테스트 13개 신규, 기존 59개 포함 72/72 PASS

**[E] 실행 — orchestrator CF-VaR 주입 + Flash Crash 보호:**
- `src/orchestrator.py`: `_build_risk()`에 KellySizer(0.25), DrawdownMonitor(8%), PortfolioOptimizer(5% VaR) 주입
  - 각 컴포넌트 try-except 처리로 초기화 실패 시 graceful degradation
- `src/risk/manager.py`: 15분 윈도우 Flash Crash Detection 추가
  - _price_history [(timestamp, price)] 추적 + 15분 윈도우 슬라이드
  - 단일 캔들 -10% 또는 15분 내 -10% 감지 → 60캔들 cooldown
- orchestrator 23, paper_trader 135, risk 225 테스트 전체 PASS

**[SIM] value_area 안정성 개선:**
- `src/strategy/value_area.py`: std_floor_pct=0.005 도입
  - 횡보장 VA 밴드 하한 → 과잉신호 억제 → SharpeStd 안정화
- `src/strategy/value_area.py`: vol_filter_mult 1.0→0.8 완화
  - 저거래 fold 33% 문제 완화
- 14/14 기존 테스트 PASS

**[F] 리서치 — 인사이트:**
- 실전 사례: 3Commas DCA 레짐 조절, Freqtrade FreqAI custom_stoploss 레짐 연동
- 레짐 필터: RegimeGuardedStrategy 래퍼 패턴 권장 (전략 코드 무수정)
- 플래시 크래시: 즉시 전량 청산 비권장, 단계적 50%→대기→나머지 패턴 효과적
- 핵심 발견: circuit_breaker.py 강화 버전이 RiskManager에 미연결 (통합 필요)
- 레짐-RiskManager 연계: adaptive_stop_multiplier()에 regime 파라미터 추가 ROI 최대
...
- `src/risk/drawdown_monitor.py`: `trailing_stop_signal()` 추가
  - 단기(20봉) MDD 속도 vs 장기(50봉) MDD 속도 비율 비교
  - `short_mdd/20 >= long_mdd/50 * accel_threshold(1.5)`이면 True
  - 낙폭 가속 조기 감지 → trailing stop 강화 신호
- `src/risk/portfolio_optimizer.py`: `cf_var_position_limit()` 추가
  - CF-VaR + Normal-VaR 기반 포지션 한도 배수 계산 (fat-tail 비율 반영)
  - CF-VaR > var_threshold 초과 시 포지션 축소 (최소 min_multiplier=0.25)
- 테스트 8개 신규 추가 (4+4), 전체 PASS

**[D] ML — DualGateADWINMonitor EWMA accuracy trend + paper_simulation CPCV 컬럼:**
- `src/ml/drift_detector.py`: `DualGateADWINMonitor.update_accuracy()` 추가
  - EWMA(alpha=0.05) 스무딩 정확도 추적
  - `ewma_accuracy`: 현재 EWMA 값 (0~1)
  - `ewma_early_warning`: 20+ 샘플 후 EWMA < 0.50이면 True (ADWIN 전 조기 경보)
  - `hard_reset()`에 EWMA 상태 초기화 포함
- `scripts/paper_simulation.py`: `run_cpcv_global()` + CPCV 리포트 섹션 추가
  - 전체 데이터셋에서 WalkForwardTrainer CPCV 1회 실행
  - 리포트에 "ML 예측 가능성 (CPCV)" 섹션 포함 (avg_test_acc, folds, PASS/FAIL)
  - 원격 환경 SSL 차단 시 "N/A" 출력
- 테스트 5개 신규 추가

**[SIM] Paper WF (BTC/ETH/SOL, 4윈도우, 1h봉, Block Bootstrap):**
- 0/22 PASS (SSL 차단 → 합성 데이터 한계)
- TOP by rank: narrow_range(76.1, PF=1.61), elder_impulse(73.6), momentum_quality(73.0)
- narrow_range: AvgPF=1.61, AvgTrades=54, AvgMDD=5.7% — PASS 기준 근접
- CPCV: N/A (합성 데이터에서 ML 학습 실패)

**[SIM] Bundle OOS (BTC/USDT, 4h봉):**
- 0/5 PASS (합성 데이터)
- value_area: SharpeStd=6.589 (불안정), 저거래 fold 33%
- narrow_range: 저거래 fold 44% (trades<3) 지속 — min_oos_trades 조정 또는 신호 조건 완화 필요
- cmf/wick_reversal: IS Sharpe 전부 음수 → 합성 데이터 한계

**[F] 리서치 — 인사이트:**
- PortfolioOptimizer CF-VaR 통합: fat-tail 위험이 높을수록 포지션 자동 축소 (최대 75%)
- EWMA early warning: ADWIN이 드리프트 선언 전에 accuracy 추세 모니터링 가능
- narrow_range 4h 저거래 지속 → 신호 조건 완화 (예: rsi 필터 제거) 또는 timeframe 상향 검토
...
- 원인: BTC 가격 ~50000에서 ATR ≈ 500-2000 (절대값), _ATR_HIGH=10 → atr_valid 항상 False
- 수정 후: volume_breakout 0 trades → BTC 기준 80 AvgTrades 회복
- F(리서치) 분석 결과: 절대 ATR 유효율 0.0% vs 비율 ATR 유효율 97.4% (Python 검증)

**[C] 데이터 — WebSocket feed 재연결 중복 캔들 방지:**
- `src/data/websocket_feed.py`: `_last_candle_timestamp_ms` 추가, `_process_message`에서 `ts <= last_ts`이면 skip
- 재연결 시 동일 타임스탬프 캔들이 두 번 추가되는 버그 방지
- `_lock` 안에서 `_last_candle_timestamp_ms` 업데이트 (스레드 안전)

**[B] 리스크 — DrawdownMonitor 단기 rolling_mdd 추가:**
- `src/risk/drawdown_monitor.py`: `DrawdownStatus.rolling_mdd_short_pct` 필드 추가 (20봉 윈도우)
- `update()` 에서 `rolling_mdd(window=20)` 계산 → 단기(20봉) vs 장기(50봉) MDD 비교 가능
- 조기 경보 신호: rolling_mdd_short >> rolling_mdd_pct이면 최근 급락 시작 신호

**[F] 리서치 — GBM 조건 출현율 분석:**
- ema20>ema50 uptrend: 48.8% ± 16.0%, close>ema20: 49.1% ± 8.6% (거의 동일)
- 실제 0 trades 원인: uptrend 조건이 아니라 ATR 절대값 필터(0% 유효) → 수정 완료
- price_cluster 2% 수정 효과: AvgTrades 6 (여전히 낮음, 추가 완화 검토 필요)

**[SIM] Paper WF (BTC/ETH/SOL, 4 윈도우, 1h봉):**
- 0/22 PASS (합성 GBM 한계)
- volume_breakout 복구: 0 trades → BTC 80, ETH/SOL 유사 수준
- TOP 3 (BTC): price_action_momentum(+217%, S=9.19), momentum_quality(+117%, S=7.83), supertrend_multi(+102%, S=8.05)
- price_cluster AvgTrades=6 (여전히 낮음)

**[SIM] Bundle OOS (BTC/USDT, 4h봉):**
- 0/5 PASS (합성 GBM 한계)
- narrow_range 저거래 fold 44% > 40% (구조적 4h봉 신호 빈도 문제)
- value_area fold 4/9에서 PASS (가장 많음)
...
- `paper_simulation.py`: 심볼별 `hash(symbol) % (2**31)` 기반 고유 seed 생성
- BTC=632087407, ETH=1692492215, SOL=1156450451 — 이전 동일 seed(42) 문제 해결
- `src/data/` graceful import 확인: ccxt try/except 이미 적용됨, 추가 수정 불필요

**[B] Risk — VaR/CVaR fallback 검증 + streak recovery 테스트:**
- `test_portfolio_optimizer.py`: TestScipyFallbackVarCvar 7개 테스트 추가 (scipy vs numpy 차이 1~2% 이내)
- `test_drawdown_monitor.py`: TestStreakRecoveryGraceSeconds 7개 테스트 추가 (14400초 후 자동 초기화)
- `drawdown_monitor.py`: to_dict()에 streak_recovery_grace_seconds 직렬화 누락 버그 수정

**[SIM] Bundle OOS 분석 (--min-trades 2):**
- 5개 전략 모두 FAIL, narrow_range fold 1,6만 의미있는 PASS (총 8 trades)
- value_area 4/9 PASS지만 평균 3.3 trades → PF=999.99 artifact, 통계 무의미
- 근본 원인: fold당 거래 수 부족 (2~6 trades, 최소 15 필요)

**[F] Research — 과적합 극복 + NR7 전략 리서치:**
- IS Sharpe는 OOS 예측 불가 (R²<0.025) — 학술적 확인
- 파라미터 2개 EMA 크로스가 81개 조합 전부 OOS 양수 (단순 전략 우위)
- NR7 크립토: 방향 필터 없이는 fake breakout으로 구조적 실패
- NR7+필터 1개: PF 2.35, MDD 13% (기준 초과 가능)
- BTC mean reversion은 저점에서만 유효

**테스트:** risk 285 passed, 전체 1318 passed
...
- `features.py`: OnChainFeatureStub 클래스 — exchange_netflow, sopr, defi_tvl 합성 폴백
- enrich_df(df)로 기존 DataFrame에 3개 온체인 컬럼 추가
- 테스트 116 passed

**[E] Execution — PaperTrader KellySizer 통합 + Tiered Slippage:**
- PaperTrader에 kelly_sizer 파라미터 추가: BUY시 compute_dynamic(), SELL시 record_trade() 자동 호출
- VolTargeting + KellySizer 동시 사용 가능 (VolTargeting 먼저 → Kelly 적용)
- Tiered Slippage: BTC/ETH 0.05%, SOL/BNB 등 0.20%, 기타 1.0% (리서치 수치 반영)
- classify_symbol_tier() + use_tiered_slippage 플래그
- 테스트 13개 추가 (85/85 passed, 관련 368/368 passed)

**[SIM] Paper Simulation 분석:**
- 실데이터 22전략×3심볼 = 66개 모두 FAIL
- value_area ETH에서 Sharpe 0.92로 최근접 (거래수 13 부족)
- 시뮬 O(n²) 병목, 합성 동일 seed(42), IS→OOS 괴리 심각
- 하위 전략: lob_maker(-12.3%), price_action_momentum(-11.8%)

**[F] Research — ML 트레이딩봇 프로덕션 배포 사례:**
- 성공: 하이브리드(DL+rule) 구조, 성능 트리거 재학습(ADWIN), 다중 레짐 검증
- 실패: 단일 ML 모델, 과다 파라미터(5개↑), 단일 레짐 최적화
- IS 승률 80%+는 과적합 신호 — OOS 통과율과 역상관
- 액션: WFA 모드 강화, 피처 수 감사, ADWIN 재학습 트리거 활성화

**테스트:** 전체 passed (ML 116, paper_trader 85, 관련 368)
...
- EmaCrossStrategy/DonchianBreakoutStrategy 파라미터 최적화 단위 테스트 4개 추가
  - test_optimize_ema_cross_uses_params(): DEFAULT_GRIDS 범위 내 파라미터 검증
  - test_optimize_donchian_uses_params(): channel_period 유효성 검증
  - test_ema_cross_dynamic_params(): fast/slow span 다른 조합이 다른 EMA 생성 확인
  - test_donchian_dynamic_params(): channel_period=10 vs 기본값(20) 채널 값 다름 확인
- make_df() 확장: rsi14, vwap, ema9/20/21/50, volume, donchian_high/low 컬럼 추가

**[C] Data & Infrastructure — 합성 데이터 레짐 개선:**
- make_synthetic_data() 트렌드/레인지/변동성 폭발 레짐 포함으로 완전 개선
  - 트렌드 블록(120~180봉): mu=±0.002~0.004, sigma=0.012~0.018
  - 레인지 블록(100~150봉): mu=0, sigma=0.006~0.010
  - 변동성 폭발 블록(50~80봉): sigma=0.035~0.055
  - GARCH-like volatility clustering 추가
  - 레짐 지속성 강화 (같은 레짐 반복 후 전환)
  - 볼륨과 변동성 상관관계 개선

**[F] Research — IS 최적화 효과 측정:**
- walk_forward.py: _optimize_in_sample()에 파라미터별 IS Sharpe 분포 로깅 추가
  - param_is_sharpes dict: {param_key: sharpe} 수집
  - DEBUG 레벨: 전체 파라미터 랭킹 출력
  - INFO 레벨: best/worst/spread 요약
- WalkForwardResult.last_is_sharpe_dist 필드: 마지막 윈도우 IS 분포 보관
- 윈도우별 IS/OOS gap 로깅 추가 (oos_vs_is_gap)
- test_is_optimization_improves_sharpe() 테스트 추가

**[TEST] 테스트 결과:**
- 7591 passed, 23 skipped (Cycle 184 대비 +9 증가)

**[SIM] 시뮬레이션 결과 (Synthetic data — Bybit SSL 차단):**
...
## [2026-05-20] Cycle 184 — D (ML) + E (실행) + F (리서치) + SIM

**[D] ML & Signals — WalkForward factory 버그 수정:**
- EmaCrossStrategy: `__init__(fast_span, slow_span)` 추가, `_get_ema_values()` 동적 계산 메서드 추가
  - 기본값(20,50) + 컬럼 존재 시 pre-computed 사용 (하위 호환), 파라미터 변경 시 동적 계산
- DonchianBreakoutStrategy: `__init__(channel_period)` 추가, `_get_channel_values()` 동적 계산
- walk_forward.py `optimize_ema_cross()`: `factory(params)` → `EmaCrossStrategy(fast_span=..., slow_span=...)`
- walk_forward.py `optimize_donchian()`: `factory(params)` → `DonchianBreakoutStrategy(channel_period=...)`
- **핵심 버그 수정**: IS 그리드 서치가 이제 실제로 다른 파라미터 조합을 테스트함

**[D] ML — OOS Sharpe std 필터 추가 (RollingOOSValidator):**
- `BundleOOSResult`에 `oos_sharpe_std` 필드 추가 (기본값 0.0)
- `RollingOOSValidator.OOS_SHARPE_STD_MAX = 1.5` 상수 추가
- `validate()` 끝에 std > 1.5이면 `all_passed=False` + fail_reasons 추가
- 결과: bundle_oos 리포트에 std 수치 표시, 불안정 전략 자동 필터

**[E] Execution — 거래 0건 전략 파라미터 완화:**
- volume_breakout: `_ATR_LOW 0.3→0.1`, `_ATR_HIGH 5.0→10.0` (거래 빈도 증가 기대)
- dema_cross: 거리 필터 `1.0%→0.5%` (크로스 신호 증가 기대)
- price_cluster: `_BOUNCE_THRESHOLD 0.002→0.005` (신호 범위 2.5배 확대)
- test_volume_breakout.py: ATR 경계값 테스트 업데이트 (0.2→0.05, 5.5→11.0)

**[F] Research — DSR 기준 재검토:**
- OOS std 필터 적용 후 bundle_oos 결과: 5/5 전략이 std > 1.5로 추가 FAIL 발생
- OOS std 범위: 3.16~6.15 (임계값 1.5보다 훨씬 높음)
- 합성 데이터 환경에서는 IS 최적화 개선 효과 측정 불가 — 실제 데이터 필요

**[TEST] 테스트 결과:**
- 7582 passed, 17 skipped (수정 전 대비 동일 수준 유지)

**[SIM] 시뮬레이션 결과 (Synthetic data, Bybit SSL blocked):**
- paper_simulation (1h, BTC, 22 strategies, 8 windows): 0/22 PASS
- bundle_oos (4h, dry-run, 5 strategies, 9 folds): 0/5 PASS (OOS std > 1.5 추가 필터 작동 확인)
...
- `DataFeed.detect_and_cache_regime()` + `fetch_with_regime()`: 캔들→레짐→캐시 원스텝
- `RegimeAwareFeatureBuilder.build_with_cached_regime()`: DataFeed 캐시 활용 학습/추론
- 지연 임포트로 순환 의존성 방지
- 16개 E2E 통합 테스트, 64/64 PASS

**[B] BayesianKellyPositionSizer 구현:**
- Beta prior (α=2, β=3), 매 거래 posterior 업데이트
- Kelly: f* = (mean*(1+b)-1)/b × fractional(0.33), max 10%
- Warmup: 50거래 미만 → 고정 0.5%
- 23개 테스트 추가, 54/54 PASS

**[SIM] 전략 파라미터 민감도 + 레짐 스위칭:**
- 133개 backtest 테스트 PASS, 0 regression
- 77% PASS 전략이 실데이터 거래수(30회) 미달 → 레짐 스위칭으로 4~5x 거래 증가 가능
- ROCMACross: 과최적화 위험 LOW (5개 표준 파라미터)
- 레짐 특화: Trend followers(Sharpe 5.77) BULL/BEAR 최적, Breakout(PF 2.41)

**[F] 리서치: River ADWIN + Telegram 아키텍처:**
- ADWIN delta: 금융 시계열 0.05~0.1 (기본값 0.002는 FP 과다)
- 이중 게이트: 피처별 ADWIN(조기경보) + 모델출력 ADWIN(확정) → FP 대폭 감소
- Warm Start 재학습: 기존 가중치 초기값 + 최근 30~90일 fine-tuning
- Telegram: 즉시/큐 분리, token bucket 25msg/s, 429시 retry_after Redis 공유
...
- `src/exchange/health_check.py`: 5분 간격 liveness, 4단계 상태(HEALTHY/DEGRADED/DISCONNECTED/PROTECTED)
- 자동 재연결 최대 3회 (exponential backoff), 실패 시 보호 모드(주문 차단)
- `src/exchange/notifier.py`: Critical/Silent/Suppress 3계층 알림 스켈레톤 (Telegram 스텁)
- `scripts/live_paper_trader.py`: HealthChecker 통합 (tick에서 체크, PROTECTED 시 스킵)
- 테스트 33개 추가 (health_check 23개 + notifier 10개)

**[A] 품질: 15개 실패 테스트 수정 → 0 failed:**
- `test_result_split_info_populated`: calibration n_cal 합계 검증 수정
- `test_generate_buy/sell_signal_without_model`: MLSignalGenerator auto-load mock 적용
- `test_connect_calls_check_permissions`: _timeout_ms 등 속성 누락 추가
- `test_ml_rf_min_trades`: auto-load mock 수정
- pyyaml 설치로 registry 테스트 10건 + collection error 5건 해소
- 최종: 7188 passed, 0 failed, 17 skipped

**[F] 리서치: Health Check + API 장애 사례:**
- Hyperliquid 27분 장애(2025.07), XTB 수 시간 장애 → Kill Switch 독립성 필수
- 권장 인터벌: API 5분, WebSocket 10~30s, 포지션 5분, Watchdog 15분
- Exponential backoff: base=1s, max=30s, full jitter, 최대 10회
- Graceful Degradation 3단계 + 성공 스택: Prometheus+Grafana+Telegram
...
- `MAX_LOSS_PCT = 0.50` 상수 + `max_loss_pct` 파라미터 추가
- `_check_max_loss()` 메서드: 초기자본 대비 50% 이상 손실 시 `_halted=True`, CRITICAL 로깅
- `tick()` 최상단에서 halt 체크 — 한번 중단되면 수동 재시작 필요
- `min_accuracy` 0.52 → 0.55 상향 (AccuracyDriftMonitor)
- 테스트 3건 추가 (halt 트리거/미트리거/복구불가), 37 PASS

**[A] 품질: FeatureBuilder 테스트 커버리지 대폭 향상:**
- 커버리지 갭 분석: `src/ml/features.py`가 핵심 모듈인데 전용 테스트 0건
- `tests/test_feature_builder.py` 29개 테스트 신규 작성
  - build() 기본 동작, NaN/inf 방어, 14개 피처 컬럼 검증
  - 3-class/binary/triple_barrier 레이블 모드 각각 검증
  - 선택적 피처 (btc_close_lag1, delta_fr, fr_oi_interaction) 조건부 생성
  - 엣지케이스 (상수 가격, 짧은 데이터, 기본 파라미터)
- 7,057 passed (+29), 36 skipped, 기존 실패 외 신규 실패 없음

**[F] 리서치: ML 드리프트 대응 + 소자본 운영 현실:**
- PSI > 0.2가 금융 업계 표준 재학습 트리거 (이미 이전 리서치와 일치)
- 다시간 윈도우 앙상블(30/60/90일) → 드리프트 시 최신 모델 가중치 자동 상승
- 폴백 전략 필수: ML 신뢰도 낮으면 규칙 기반으로 자동 전환
- 소자본: Bybit maker 0.020%, 4h 스윙이 소자본에 유리, 슬리피지 레짐별 가변 필요
- 권장: PSI 모니터 구현, XGBoost 다시간 앙상블, 수수료 현실화(taker 0.055%)
...
## [2026-04-20 02:45 UTC] Cycle 162 Dispatched — B + D + SIM + F
Categories: B + D + SIM + F. Briefing: CURRENT_CYCLE_BRIEFING.md

## [2026-04-20 02:45 UTC] Cycle 163 Dispatched — E + A + SIM + F
Categories: E + A + SIM + F. Briefing: CURRENT_CYCLE_BRIEFING.md

## [2026-04-20 02:50 UTC]
Pipeline: preflight
Status: ERROR
Signal: N/A
Risk: N/A
...
## [2026-04-20 16:41 UTC] Cycle 174 Dispatched — C + B + SIM + F
Categories: C + B + SIM + F. Briefing: CURRENT_CYCLE_BRIEFING.md
...
## [2026-05-22 00:37 UTC] Cycle 194 Dispatched — C + B + SIM + F
Categories: C + B + SIM + F. Briefing: CURRENT_CYCLE_BRIEFING.md

## [2026-05-25 14:13 UTC] Cycle 195 Dispatched — D + E + SIM + F
Categories: D + E + SIM + F. Briefing: CURRENT_CYCLE_BRIEFING.md
...
## [2026-05-25 14:53 UTC] Cycle 209 Dispatched — C + B + SIM + F
Categories: C + B + SIM + F. Briefing: CURRENT_CYCLE_BRIEFING.md

## [2026-05-25 15:00 UTC]
Pipeline: preflight
Status: ERROR
Signal: N/A
Risk: N/A
...
## [2026-05-26 20:35 UTC] Cycle 218 Dispatched — E + A + SIM + F
Categories: E + A + SIM + F. Briefing: CURRENT_CYCLE_BRIEFING.md
...
## [2026-05-26 20:50 UTC] Cycle 220 Dispatched — D + E + SIM + F
Categories: D + E + SIM + F. Briefing: CURRENT_CYCLE_BRIEFING.md
...
## [2026-05-27 15:38 UTC] Cycle 225 Dispatched — D + E + SIM + F
Categories: D + E + SIM + F. Briefing: CURRENT_CYCLE_BRIEFING.md

## [2026-05-27 15:43 UTC]
Pipeline: preflight
Status: ERROR
Signal: N/A
Risk: N/A
