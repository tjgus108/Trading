...
tests/test_feed_boundary.py (Cycle418 C, TestIndicatorBoundaryC418):
1. `test_ema20_slope_near_zero_for_constant_prices`: 상수 가격 50봉 → ema20 수렴 → ema20_slope≈0 검증
2. `test_vwap20_equals_close_for_constant_ohlcv`: 상수 가격+거래량 → vwap20(롤링20봉)=close 검증
3. `test_macd_hist_near_zero_for_constant_prices`: 상수 가격 → EMA12=EMA26=EMA9=price 수렴 → macd_hist≈0 검증

**[B(리스크)] DrawdownMonitor 복합 케이스 3개** (+3)

tests/test_drawdown_monitor.py (Cycle418 B):
1. `test_block_entry_and_sharpe_decay_block_dominates`: BLOCK_ENTRY(0.0)+sharpe_decay(0.5) → get_size_multiplier=0.0 (BLOCK이 decay 무력화)
2. `test_streak_and_atr_elevated_no_mdd_no_sharpe`: streak(0.5)+ATR elevated(0.5), MDD/sharpe 정상 → get_size_multiplier=0.5
3. `test_block_entry_and_atr_elevated_block_dominates`: BLOCK_ENTRY(0.0)+ATR elevated(0.5) → get_size_multiplier=0.0 (BLOCK 지배)

**코드 개선 1건**:
1. `walk_forward.py` dema_cross 섹션에 Cycle418 F 2/8 Consistency ceiling 구조 분석 주석 추가

**[F(리서치)] dema_cross 2/8 Consistency ceiling 원인 확정**

분석 결과 (BTC 1h paper_sim: Sh=0.85, PF=1.38, Trades=26, 2/8 Consistency, Rank 3):
- **2/8 원인**: RANGING(47.3%) → DEMA(8,20) 크로스가 방향성 없는 노이즈 신호 생성
  - rsi_dir_filter=True+threshold=40이 일부 차단하나 RANGING 전체 제거 불가
  - PASS 2윈도우 = BTC TREND_UP 구간(2023 Q4/2024 Q1) — 볼륨 스파이크+추세 일치
  - FAIL 6윈도우 = RANGING/BEAR에서 false cross 빈발 → PF 구조적 <1.5
- **bb_width_min_filter 탐색 가능성**: 0.04가 이미 BB squeeze(p25=0.041) 차단 중, 0.05→dead(Cycle375 C). 상향 시 Trades<15 위험
- **PF gap=0.12 (1.38→1.50)**: 20+ 사이클 탐색으로 달성 불가 확인됨 (Cycle377 F 결론 재확인)
- **결론**: dema_cross 2/8 ceiling은 frama(0/8), price_cluster(2/8)와 동일한 RANGING 47.3% BTC 1h 구조적 한계. 추가 탐색 완전 종료 재확인.

**시뮬레이션 결과**:
- Paper Sim (1h BTC CSV fallback, 2026-07-12): roc_ma_cross 1/19 PASS 유지 (Sh=1.81, PF=2.02, Trades=14, 4/8)
  - dema_cross rank3: Sh=0.85, PF=1.38, Trades=26, 2/8 (2/8 Ceiling 구조적 확정)
  - 전략 추가/제거 없음, 결과 변화 없음
- Bundle OOS (4h 합성데이터, 2026-07-12): 합성 데이터 fallback (bybit/binance/okx 전부 차단)
  - 합성 데이터 매 실행마다 상이 → 리포트 미업데이트, 기존 BUNDLE_OOS_REPORT.md (2026-07-08, 5/5) 유지

**총 테스트**: 8716 총계 (8693 passed, 23 skipped) [Cycle417 대비 +6]

---

## [2026-07-12] Cycle 417 — B(리스크) + D(ML) + F(리서치)

**[B(리스크)] CircuitBreaker 추가 미커버 케이스 3개** (+3)

tests/test_circuit_breaker.py (Cycle417 B):
1. `test_flash_crash_trigger_cleared_by_reset_daily`: 플래시 크래시(10% 캔들 변동) 트리거 후 reset_daily()로 해제 확인 (reason='플래시' → is_triggered=False)
2. `test_check_returns_no_trigger_when_balances_invalid`: daily_start_balance=0 또는 peak_balance=0 → triggered=False 조기 반환
3. `test_flash_crash_takes_priority_over_daily_drawdown`: 플래시 크래시 + 일일 낙폭 동시 → 플래시 크래시 우선 처리 (reason에 '플래시', '일일' 없음)

**[D(ML)] optimize_price_cluster 함수 추가 + 테스트 3개** (+3)

walk_forward.py: `optimize_price_cluster()` 함수 신규 추가 (DEFAULT_GRIDS["price_cluster"] 활용)
tests/test_phase_d.py (Cycle417 D, TestOptimizePriceCluster):
1. `test_optimize_price_cluster_returns_wf_result`: 기본 호출 → WalkForwardResult, strategy_name='price_cluster'
2. `test_optimize_price_cluster_fold_pass_rate_type`: fold_pass_rate는 None이거나 [0.0, 1.0] float
3. `test_optimize_price_cluster_avg_oos_sharpe_is_float`: avg_oos_sharpe float 타입 검증

**코드 개선 2건**:
1. `WalkForwardResult.avg_oos_trades`: fold 평균 OOS 거래 수 필드 추가 (거래 0건 fold 비율 진단용 — supertrend_multi fold 4: 0 trades 패턴 대응)
2. `optimize_frama()` docstring에 Cycle417 F 분석 결과 주석 추가

**[F(리서치)] frama 0/8 Consistency ceiling 원인 확정**

분석 결과 (BTC 1h CSV 실데이터, Sh=0.44, PF=1.11, Trades=65, 0/8 Consistency):
- **0/8 원인**: PF 구조적 ceiling. fail reason = 'profit_factor 1.00 < 1.5 (x2), profit_factor 0.89 < 1.5 (x1)' — PF가 avg 1.11, 개별 window에서도 1.0~1.11 범위 ceiling
- **SharpeStd=1.23** (매우 낮음 — 안정적) 이지만 Sharpe 자체가 1.0을 넘지 못함 (가장 좋은 window도 Sh < 1.0)
- **WFO 27-combo 분석**: period×rsi_period×weak_rsi_buy_max 최적화해도 OOS PF < 1.5. IS에서 선택된 params가 OOS에서 항상 PF ceiling에 막힘
- **Consistency=0/8 이유**: BUY/SELL 신호 WR=34.5% — FRAMA 적응형 필터가 RANGING에서 동일 PF 패턴 반복, TREND_UP 구간에서만 PF>1.5 가능하나 8개 window 중 해당 구간 없음
- **결론**: frama 추가 파라미터 탐색 완전 종료. PF 구조적 ceiling 확인. 보조 전략으로 보존.

**시뮬레이션 결과**:
- Paper Sim (1h BTC CSV, 2026-07-12): roc_ma_cross 1/19 PASS 유지 (Sh=1.81, PF=2.02, Trades=14, 4/8)
- Bundle OOS (4h 합성데이터): 0/5 PASS (supertrend_multi Score=92.0 p100 최고; 합성데이터 → 리포트 미저장)
  - supertrend_multi: WFE=0.835, Sh=4.337, 합성 최고 성능 — 실거래소 데이터 검증 필요

**총 테스트**: 8710 총계 (8687 passed, 23 skipped) [Cycle416 대비 +6]

---

## [2026-07-11] Cycle 416 — B(리스크) + D(ML) + F(리서치)

**[B(리스크)] DrawdownMonitor 복합 케이스 3개 추가** (+3)

tests/test_drawdown_monitor.py (Cycle416 B):
1. `test_kelly_fraction_and_sharpe_decay_compound`: kelly_fraction_multiplier(0.5, MDD>kelly_reduce) + sharpe_decay(0.5) 독립 동시 — get_size_multiplier=0.5 (sharpe_decay), get_kelly_fraction=0.5 (별개 레이어)
2. `test_streak_and_sharpe_decay_no_mdd_atr`: 3연속 손실(streak=0.5) + sharpe_decay(0.5), MDD/ATR 정상 → get_size_multiplier=min(0.5,1,1,0.5)=0.5
3. `test_sharpe_decay_recovery_while_high_vol_daily_limit_remains`: HIGH_VOL 설정 후 sharpe_decay 트리거→회복; size_mult 1.0 복원, HIGH_VOL 일일 한도(2%) 유지 독립성 검증

**[D(ML)] optimize_donchian 경계값 + select_features_pfi 반환 검증 5개** (+5)

tests/test_phase_d.py (Cycle416 D, TestOptimizeDonchian + TestSelectFeaturesPfiOutputValidation):
4. `test_optimize_donchian_best_params_is_dict`: best_params가 dict이고, 비어있지 않으면 'channel_period' 포함
5. `test_optimize_donchian_avg_oos_sharpe_is_float`: avg_oos_sharpe는 float 타입
6. `test_select_features_pfi_all_from_input_columns`: 반환 피처명이 모두 X_train.columns 내에 있음
7. `test_select_features_pfi_no_duplicates`: 반환 목록에 중복 없음

**[F(리서치)] roc_ma_cross AvgTrades=14 경계 구조적 분석 완료**

분석 결과 (BTC 1h, Sh=1.81, PF=2.02, Trades=14, 4/8 PASS):
- **PASS 4/8 윈도우 구간**: 2023 Q4(BTC 27k→44k) + 2024 Q1(44k→73k) — BTC 급등 구간에서 볼륨 스파이크 동반 → vol_ratio≥1.2 충족 신호 빈발 → Trades≥15 달성
- **FAIL 4/8 윈도우 원인**: 2023 H1(15k→30k 저거래량 회복) + 2024 Q2(73k→65k 조정) — vol_ratio at signals=0.89-0.97(<1.2) → 대부분 신호 차단 → Trades=10-12 (경계 미달)
- **AvgTrades=14 ceiling 구조적 원인**: BTC 1h 60d 윈도우 × vol_ratio_min=1.2 필터 = ~10% 통과율; ROC(12) 크로스 신호 ~120-140개 중 ~14개만 통과
- **결론**: AvgTrades=14는 구조적 ceiling. vol_ratio 조정 불가(1.1→2/8 Cycle382), 필터 추가 불가(모두 역효과). **4/8 Consistency가 1h BTC vol_ratio=1.2 조합의 구조적 최적점. roc_ma_cross 탐색 완전 종료.**
...
## [2026-07-11] Cycle 415 — A(품질) + C(데이터) + F(리서치)

**[A(품질)] apply_wfe 미커버 3케이스 + 테스트 추가** (+3)

tests/test_backtest_engine.py (Cycle415 A):
1. `test_apply_wfe_below_min_wfe_sets_fail_and_not_passed`: IS>0, 0<wfe<MIN_WFE(0.5) → fail_reasons 추가 + passed=False
2. `test_apply_wfe_is_exactly_neg1_boundary_gives_wfe1`: IS=-1.0 경계값 → not < -1.0 → wfe=1.0 (소폭 음수 브랜치)
3. `test_apply_wfe_oos_exactly_1p5_when_is_strong_neg_gives_zero`: IS=-2.0, OOS=1.5 → not >1.5 → wfe=0.0 (경계)

**[C(데이터)] feed 지표 경계값 3케이스 추가** (+3)

tests/test_feed_boundary.py (Cycle415 C, TestIndicatorBoundaryC415):
4. `test_rsi14_bounded_0_to_100`: 진동 가격 50행 → rsi14 비NaN 값 [0,100] 범위 검증
5. `test_donchian_high_gte_donchian_low`: 50행 정상 → donchian_high >= donchian_low (비NaN 쌍)
6. `test_vwap_equals_close_for_constant_ohlcv`: 상수 OHLCV → vwap = close (typical=close 수학적 일관성)

**[F(리서치)] price_cluster 2/8 Consistency ceiling 구조적 원인 확정**

분석 결과 (rank 3, BTC 1h Cycle415: Sh=1.06, PF=1.32, Trades=35, MDD=7.7%, 2/8 Consistency):
- **PF=1.32 (<1.5)**: avg PF가 1.5 미달 → 2/8 PASS 윈도우만 PF>=1.5 달성
- **2/8 ceiling 구조적 원인**:
  ...
  - vol_regime_filter=True 적용해도 TREND_UP 일부 구간 신호 발생 → PF<1.5
  - RANGING 47.3% BTC 1h에서 cluster bounce는 2/8 ceiling이 구조적 한계
- **코드 개선**: walk_forward.py price_cluster grid atr_bounce_factor [0.0,0.3,0.5,1.0] → [0.5] 단일값
  - 탐색 종료된 dead 값 제거 → 72 combos → 18 combos (75% 감소), IS 최적화 속도 개선
  - Cycle415 F 분석 주석 추가 (구조적 한계 근거 문서화)

**시뮬레이션 결과**:
- Paper Sim (1h BTC, 2026-07-11): roc_ma_cross PASS 유지 (Sh=1.81, PF=2.02, Trades=14, 4/8) — 1/19 PASS 유지
- Bundle OOS: SSL 차단으로 합성 데이터 fallback → 리포트 보존 (기존 5/5 PASS 유지)

**테스트**: 8700 총계, 8677 passed (+6 신규)
...
tests/test_feed_boundary.py (Cycle413 C, TestIndicatorBoundaryC413):
1. `test_atr14_near_zero_for_constant_ohlc`: 모든 봉 OHLC 동일(true range=0) → atr14 < 1.0 검증
2. `test_ema50_slower_than_ema20_on_price_spike`: 가격 급등 후 ema50이 ema20보다 close와의 거리 더 멀어야 함(반응 속도)
3. `test_return_5_sign_matches_trend`: 상승 추세 → return_5 양수, 하락 추세 → 음수 부호 검증

**[B(리스크)] trailing_stop_signal 경계 + kelly+mdd_warn 복합 3개** (+3)

tests/test_drawdown_monitor.py (Cycle413 B):
4. `test_trailing_stop_signal_short_window_boundary`: rolling_window=40 → short_window=min(20,20)=20 경계값, 급락 시 True 검증
5. `test_trailing_stop_signal_threshold_one_uniform_decline`: accel_threshold=1.0 → 균일 하락에서도 short_rate>=long_rate→True
6. `test_kelly_fraction_multiplier_and_mdd_warn_compound`: MDD 9% (warn=8%, block=10%): kelly=0.5 + mdd_level=WARN 동시 발생 복합 검증

**[F(리서치)] positional_scaling BTC 1h 구조적 한계 확정**

분석 결과 (rank 12, BTC 1h, Sh=-0.38, PF=1.09, Trades=34, 1/8 Consistency):
- 근본 원인: `pullback == rally` 동일 조건 (`-threshold <= deviation <= threshold`)
  - BUY 조건의 pullback과 SELL 조건의 rally가 완전 동일 → 진입 구간 동일
  - bullish_alignment AND near_EMA20 AND 양봉 / bearish_alignment AND near_EMA20 AND 음봉
  - 방향성 에지 없음: close가 EMA20 근방 ±0.45%(BTC 1h 기준 ATR~630, EMA~42000) 이내
- Triple EMA alignment: RANGING 47.3%에서 EMA20≈EMA50≈EMA100 → alignment 드물게 충족 → Trades=34 제한
  - alignment 충족 시에도 BUY/SELL 대칭 발생 → 1/8 Consistency만 달성
- pullback_atr_mult=0.3 파라미터화 평가:
  - 상향(0.5/1.0): 진입 구간 확대, BUY/SELL 대칭성 유지 → 에지 변화 없음
  - 하향(0.1): Trades<15 위험 → 개선 불가
- **결론**: BUY/SELL 진입 구간이 구조적으로 동일한 설계 결함. pullback≠rally로 수정해야 하나 전략 파일 구조 변경 필요. 추가 탐색 금지.
- walk_forward.py DEFAULT_GRIDS["positional_scaling"]={} 이미 추가됨 (Cycle403), 유지

**시뮬레이션 결과 (Cycle 413)**:
- 1h PASS: 1/19 (roc_ma_cross — Sh=1.81, PF=2.02, Tr=14, 4/8) — 유지
- Bundle OOS: 5/5 PASS (2026-07-08 기준 유지 — SSL fallback으로 갱신 불가)
- 총 테스트: **8685 총계 (+6, 8656→8662 passed)** (test_feed_boundary.py: 52→55, test_drawdown_monitor.py: 149→152)

---

## [2026-07-10] Cycle 412 — B(리스크) + D(ML) + F(리서치)

**[B(리스크)] CircuitBreaker max_daily_trades + consecutive_loss 복합 케이스 3개** (+3)

tests/test_circuit_breaker.py (Cycle412 B):
1. `test_consecutive_loss_cooldown_takes_priority_over_daily_trade_limit`: 쿨다운 활성 + 일일거래한계 동시 → 쿨다운 reason 우선 반환 (check()순서: 쿨다운이 daily_trade_limit보다 먼저 체크됨)
2. `test_reset_daily_preserves_consecutive_loss_cooldown`: reset_daily()는 일일 통계만 초기화 — cooldown_remaining 미초기화 검증
3. `test_record_trade_during_cooldown_increments_daily_trade_count`: 쿨다운 중 record_trade_result() 호출 시 _daily_trade_count 계속 증가 검증

**[D(ML)] optimize_roc_ma_cross 미커버 필드 3개** (+3)

tests/test_phase_d.py (Cycle412 D, TestOptimizeRocMaCrossFields):
4. `test_optimize_roc_ma_cross_oos_sharpe_std_non_negative`: oos_sharpe_std >= 0.0
5. `test_optimize_roc_ma_cross_best_params_has_roc_min_abs`: best_params에 roc_min_abs 키 포함 (factory에서 전달)
6. `test_optimize_roc_ma_cross_fold_pass_rate_in_range`: fold_pass_rate ∈ [0.0, 1.0]

**[F(리서치)] momentum_quality BTC 1h 구조적 한계 확정**

분석 결과 (rank 14, BTC 1h, Sh=-1.19, PF=0.96, Trades=71, 1/8 Consistency, SharpeStd=3.29):
- paper_sim: Sh=-1.19, PF=0.96(**<1.0, 음의 엣지**), Trades=71, Consistency=1/8 → FAIL
- 근본 원인 1: `quality_score = consistency*2-1 + (acceleration>0)` → RANGING(47.3%)에서 빈발
  - consistency_buy_threshold=0.3: **DEAD PARAM** — quality_score>0.8 조건이 이미 consistency>0.4 함의 (accel=1시) 또는 >0.9 함의 (accel=0시)
  - 즉 threshold=0.3 완화가 신호 빈도에 전혀 영향 없음
- 근본 원인 2: RANGING(47.3%) BTC 1h에서 mom20>0 조건이 ~50% 충족 + quality_score>0.8 구조적 충족 → 노이즈 거래 과다
- 파라미터 개선 방향 검토:
  quality_score_buy_threshold=1.5 (0.8→1.5): accel=1 + consistency>0.75 필요 → Trades 급감 (<15 위험)
  SharpeStd=3.29 (불안정) → 파라미터 개선으로 안정화 불가능
- walk_forward.py DEFAULT_GRIDS["momentum_quality"]={} 추가 (구조적 한계 주석)
- **결론**: RANGING 47.3% BTC 1h에서 모멘텀 품질 측정 자체가 노이즈. 추가 탐색 금지.

**시뮬레이션 결과 (Cycle 412)**:
- 1h PASS: 1/19 (roc_ma_cross — Sh=1.81, PF=2.02, Tr=14, 4/8) — 유지
- Bundle OOS: 5/5 PASS (2026-07-08 기준) — 유지 (SSL fallback으로 갱신 불가)
- 총 테스트: **8679 총계 (+6, 8650→8656 passed)** (full suite: 8656 passed, 23 skipped)

---

## [2026-07-10] Cycle 411 — B(리스크) + D(ML) + F(리서치)

**[B(리스크)] DrawdownMonitor transition_cushion + set_regime 복합 케이스 3개** (+3)

tests/test_drawdown_monitor.py (Cycle411 B):
1. `test_transition_cushion_disabled_by_default_returns_one`: transition_cushion_enabled=False(기본) → confidence=0.0/0.5/1.0 전부 1.0 반환
2. `test_transition_cushion_enabled_crisis_regime_compound`: CRISIS 레짐 + cushion 활성화 + confidence<threshold → 일일 한도 2% 강화 + cushion 0.5x 복합 검증
3. `test_get_size_multiplier_atr_and_sharpe_decay_no_streak_mdd`: ATR elevated(0.5x) + Sharpe decay(0.5x), streak/MDD 없음 → get_size_multiplier()=min(1.0,1.0,0.5,0.5)=0.5

**[D(ML)] optimize 함수 strategy_name + best_params 키 검증 3개** (+3)

tests/test_phase_d.py (Cycle411 D, TestOptimizeFramaStrategyName):
4. `test_optimize_frama_strategy_name_is_frama`: optimize_frama() → strategy_name == "frama"
5. `test_optimize_dema_cross_best_params_contains_bb_width_min_filter`: optimize_dema_cross() best_params에 bb_width_min_filter 키 포함 검증
6. `test_optimize_narrow_range_strategy_name_is_narrow_range`: optimize_narrow_range() → strategy_name == "narrow_range"

**[F(리서치)] volume_breakout BTC 1h 구조적 한계 확정**

분석 결과 (rank 6, BTC 1h, Sh=-0.74, Trades=72, MDD=22.1%):
- paper_sim: Sh=-0.74, PF=0.96(**<1.0, 음의 엣지**), Trades=72, Consistency=0/8 → FAIL
- 근본 원인 1: **손절 파라미터 없음** — `atr_mult` 등 SL 파라미터 미구현, BacktestEngine 기본 max_hold_candles=48h만으로 관리
  → MDD 22.1% > 20% 한계 (x2 FAIL 이유)
- 근본 원인 2: **_SPIKE_MULT=1.5 하드코딩** — RANGING(47.3%) BTC 1h에서 낮은 임계값 → 신호 빈발(Trades=72, avg 9/window)
- 근본 원인 3: EMA50 추세 필터가 신호 조건이 아닌 **confidence 결정**에만 사용
  → RANGING에서 BUY/SELL 신호 동등 발생 → 방향성 없는 노이즈 거래 과다
- 파라미터 개선 방향 검토:
  spike_mult=2.0 (1.5→2.0): Trades 감소 (72→~40), MDD 일부 개선 가능하나 PF<1.0 구조 유지
  spike_mult=2.5+: Trades<15 위험
  ema50 추세 필터를 신호 조건으로 이동: RANGING(47.3%) 전면 차단 → Trades 더 감소
  SL 파라미터 추가: 코드 수정 필요, MDD 개선 불확실 (기존 Sh<0 = 손실 자체 문제)
- **결론**: volume_breakout RANGING 47.3% BTC 1h에서 구조적 실패 확정. Sh=-0.74, PF=0.96, 0/8 Consistency.
  - 하드코딩 파라미터: _SPIKE_MULT, _HIGH_CONF_MULT, _ATR_LOW/HIGH_PCT 모두 파라미터화 가능
  - 단 PF<1.0 (음의 엣지) + SL 부재 + RANGING 환경 → 파라미터화로 해결 불가
  - **추가 탐색 금지**. walk_forward.py DEFAULT_GRIDS["volume_breakout"]={} 추가.
- **코드 수정**: walk_forward.py DEFAULT_GRIDS["volume_breakout"]={} 구조적 한계 주석 추가

**시뮬레이션 결과 (Cycle 411)**

1h Walk-Forward (기존 Cycle 410 보고서 유효, 파라미터 변경 없음):
- 1h PASS: 1/19 (roc_ma_cross: Sh=1.81, PF=2.02, Trades=14, 4/8 Consistency)
- 1h 2위: price_cluster (Sh=1.06, PF=1.32, 2/8 FAIL)
- 1h 3위: dema_cross (Sh=0.85, PF=1.38, 2/8 FAIL)

Bundle OOS 4h (기존 5/5 PASS 유지):
- cmf: Sh=2.508, PF=1.387, PASS
- order_flow_imbalance_v2: Sh=4.345, PF=1.941, PASS
- supertrend_multi: Sh=3.892, PF=2.737, PASS
- vwap_cross: Sh=3.047, PF=1.917, PASS
- value_area: Sh=3.069, PF=1.771, PASS

**테스트 현황**: 8644 passed, 23 skipped (+6 추가)

---

## [2026-07-10] Cycle 410 — A(품질) + C(데이터) + F(리서치)

**[A(품질)] BacktestEngine apply_wfe 미커버 브랜치 3개** (+3)

tests/test_backtest_engine.py (Cycle410 A):
1. `test_apply_wfe_mild_negative_is_positive_oos_gives_one`: IS=-0.5 (소폭 음수, -1 < IS ≤ 0) + OOS=1.0 → wfe=1.0 (과최적화 아님 브랜치)
2. `test_apply_wfe_both_nonpositive_gives_zero`: IS=-2.0 + OOS=-0.5 (OOS ≤ 0 fallthrough) → wfe=0.0
3. `test_apply_wfe_zero_is_positive_oos_gives_one`: IS=0.0 (정확히 0, > 0 분기 실패 + not < -1.0) + OOS=1.2 → wfe=1.0

**[C(데이터)] DataFeed 지표 경계값 테스트 3개** (+3)

tests/test_feed_boundary.py (Cycle410 C, TestIndicatorBoundaryC410):
4. `test_ema20_slope_positive_for_sustained_uptrend`: 60봉 단조 증가 → ema20_slope 마지막 값 양수
5. `test_donchian_high_excludes_current_bar`: donchian_high shift(1) 검증 — 21봉 데이터에서 마지막 봉 high(50050)가 donchian_high에 미반영 확인
6. `test_vwap20_finite_with_uniform_price_volume`: 균일 가격/거래량 50봉 → vwap20 NaN/Inf 없음 + typical price에 수렴

**[F(리서치)] relative_volume BTC 1h 구조적 한계 확정**

분석 결과 (rank 8, BTC 1h, rvol_buy_sell=1.2):
- paper_sim: Sh=-0.99, PF=0.92(**<1.0**), Trades=64, Consistency=0/8 → FAIL
- 근본 원인 1: rvol_buy_sell=1.2는 RANGING(47.3%) BTC 1h에서 단기 거래량 노이즈로도 빈번히 충족
  → RANGING 볼륨 스파이크 = 방향성 없는 노이즈 → 추세 추종 실패 → 즉각 반전 → 음의 엣지
- 근본 원인 2: RSI<68(BUY) 허용 범위 과다 (RSI 분포의 68% 허용)
  → RANGING에서 RSI 40-60 구간이 50%+ → 거의 모든 볼륨 스파이크가 BUY 신호로 전환
- 파라미터 개선 방향 검토:
  rvol_buy_sell=1.6(기본값): Trades<15 가능 → PASS 기준 미달
  RSI<50 강화: 음의 엣지 전략에서 신호 감소 → 더 나빠짐
  bull_only=True (EMA50 필터): TREND_UP(31.3%) 전용 → Trades 더 감소
- **결론**: relative_volume 추가 탐색 금지. 구조적 한계 확정. walk_forward.py 문서화 완료.

Paper Simulation (1h BTC Cycle 410): **PASS 1/19 유지** (변화 없음)
...
Bundle OOS (4h BTC Cycle 410): **5/5 PASS 유지** (SSL 차단으로 재실행 시 synthetic fallback 방지됨, 2026-07-08 리포트 유효)

코드 변경 사항:
- `tests/test_backtest_engine.py`: apply_wfe 미커버 3개 테스트 추가 (mild_negative IS=1.0, both_nonpositive, zero IS)
- `tests/test_feed_boundary.py`: TestIndicatorBoundaryC410 클래스 3개 추가 (ema20_slope, donchian_shift, vwap20_finite)
- `src/backtest/walk_forward.py`: DEFAULT_GRIDS["relative_volume"]={} + 구조적 한계 주석 (Cycle410 F)

테스트: 8655 → 8661 총계 (+6), 8632 → 8638 passed (+6)

---

## [2026-07-09] Cycle 409 — D(ML) + E(실행) + F(리서치)

**[D(ML)] select_features_pfi() 경계값 3개 + optimize_narrow_range type 1개** (+4)

tests/test_ml_pipeline_edge_cases.py (Cycle409 D, TestSelectFeaturesPfi):
1. `test_select_features_pfi_n99_boundary_small_sample_path`: n_samples=99 (< 100 경계) → n_repeats=10 분기 진입, 비어 있지 않은 리스트 반환
2. `test_select_features_pfi_two_features_returns_both`: n_features=2, top_k=8 → k=max(2,min(8,2))=2, 두 피처 전부 반환
3. `test_select_features_pfi_selected_features_are_subset_of_columns`: 반환 피처가 모두 X.columns 원소 — 존재하지 않는 피처명 없음

tests/test_phase_d.py (Cycle409 D, TestOptimizeNarrowRange):
4. `test_optimize_narrow_range_avg_oos_sharpe_is_float`: avg_oos_sharpe 반환값 isinstance float 타입 검증

**[E(실행)] PaperConnector 미커버 실행 케이스 3개** (+3)

tests/test_exchange.py (Cycle409 E, TestPaperConnectorCycle409):
5. `test_profitable_round_trip_increases_balance`: buy @ 10000 → sell @ 11000, fee=0, 최종 잔고 > 초기 잔고 (수익 거래 정상 반영)
6. `test_multiple_buys_increase_position_cumulatively`: 연속 3번 buy × 0.5 BTC → open_positions 수량 1.5 BTC (누적 정상)
7. `test_full_fill_remaining_is_zero`: timeout_prob=0 정상 체결 → order['remaining'] == 0.0 (full fill 검증)

**[F(리서치)] price_action_momentum BTC 1h 구조적 한계 확정**

분석 결과 (rank 10, BTC 1h):
- paper_sim: Sh=-1.08, PF=0.97(**<1.0**), Trades=73, MDD=16.7%, Consistency=1/8 → FAIL
- 근본 원인 1: roc5 > 0.005 + body_strength >= 0.50 AND 조건이 RANGING(47.3%) BTC 1h에서 14%/bar 빈도로 충족 (synthetic RANGING 2000봉 실측)
- 근본 원인 2: RANGING 진입 후 즉각 반전 → 음의 엣지(-1.08)
- 파라미터화 가능: body_strength_threshold(0.50), roc5_threshold(0.005) 하드코딩
  → 강화 시 Trades 감소 + 이미 음의 엣지이므로 timing 더 나빠짐
- **결론**: 모멘텀 추종 전략이 RANGING 47.3% BTC 1h에서 구조적 실패. 추가 탐색 금지.
- walk_forward.py: DEFAULT_GRIDS["price_action_momentum"]={} 구조적 한계 주석 추가

Paper Simulation (1h BTC Cycle 409): **PASS 1/19 유지** (변화 없음)
- roc_ma_cross: PASS (4/8, Sh=1.81, PF=2.02, Tr=14, MDD=3.4%)
- price_cluster: FAIL (2/8, Sh=1.06, PF=1.32, Tr=35)
- dema_cross: FAIL (2/8, Sh=0.85, PF=1.38, Tr=26)
- price_action_momentum: FAIL (1/8, Sh=-1.08, PF=0.97, Tr=73) — 구조적 실패 확정

Bundle OOS (4h BTC Cycle 409): **5/5 PASS 유지** (2026-07-08 리포트, SSL 차단으로 재실행 시 synthetic fallback 방지됨)

코드 변경 사항:
- `tests/test_ml_pipeline_edge_cases.py`: TestSelectFeaturesPfi n_samples=99, n_features=2, subset 테스트 3개 추가
- `tests/test_phase_d.py`: TestOptimizeNarrowRange avg_oos_sharpe float 타입 테스트 추가
- `tests/test_exchange.py`: TestPaperConnectorCycle409 3개 추가 (수익 round-trip, 누적 포지션, remaining=0)
- `src/backtest/walk_forward.py`: DEFAULT_GRIDS["price_action_momentum"]={} + 구조적 한계 주석

테스트: 8648 → 8655 총계 (+7), 8625 → 8632 passed (+7)

---

## [2026-07-09] Cycle 408 — C(데이터) + B(리스크) + F(리서치)

**[C(데이터)] DataFeed 지표 경계값 테스트 3개** (+3)

tests/test_feed_boundary.py (Cycle408 C, TestIndicatorBoundaryC408):
1. `test_ema200_lags_ema20_on_sudden_spike`: 240봉 고정 + 10봉 급등 → |ema200-close| > |ema20-close| (ema200 더 느린 반응 검증)
2. `test_bb_width_zero_for_constant_close`: 50봉 동일 가격 → std=0 → bb_width=0 수학적 검증
3. `test_macd_hist_positive_in_sustained_uptrend`: 100봉 단조 증가 → macd_hist > 0 (fast EMA > slow EMA → signal lag 검증)

**[B(리스크)] get_size_multiplier 복합 케이스 3개** (+3)

tests/test_drawdown_monitor.py (Cycle408 B):
4. `test_atr_elevated_and_mdd_warn_compound`: ATR elevated(0.5x) + MDD WARN(0.5x) → min(1.0, 0.5, 0.5, 1.0)=0.5
5. `test_get_size_multiplier_all_four_factors_compound`: streak(0.5)+MDD WARN(0.5)+ATR elevated(0.5)+Sharpe decay(0.5) → min=0.5
6. `test_trend_down_regime_does_not_change_daily_limit`: TREND_DOWN 레짐 일일 한도 3% 유지 (HIGH_VOL/CRISIS만 2% 강화)

**[F(리서치)] htf_ema 구조적 한계 분석 + walk_forward.py 문서화**

htf_ema 분석 (rank 13, Cycle408):
- BTC 1h: Sh=-0.72, PF=0.91(**<1.0=음의 엣지**), Trades=43, Consistency=0/8
- 근본 원인 1: HTF 시뮬레이션 부정확 — iloc[::4] 다운샘플링이 실제 4h OHLCV와 불일치
  (open/high/low/close가 1h 샘플링 기준이라 EMA 값이 real 4h EMA와 다른 proxy)
- 근본 원인 2: BTC 1h RANGING 47.3% → EMA9 크로스 신호가 양방향 빈발
  htf_rising AND cross_above AND cross_valid AND rsi_ok 조건으로도 절반 이상 차단 불가
- 파라미터화 가능 요소 없음: htf span=21, ema9=9, rsi thresh 하드코딩
- **결론**: htf_ema 추가 탐색 금지 → walk_forward.py DEFAULT_GRIDS["htf_ema"]={} 구조적 한계 주석 추가

Paper Simulation (1h BTC Cycle 408): **PASS 1/19 유지** (변화 없음)
- roc_ma_cross: PASS (4/8, Sh=1.81, PF=2.02, Tr=14, MDD=3.4%)
- price_cluster: FAIL (2/8, Sh=1.06, PF=1.32, Tr=35)
- dema_cross: FAIL (2/8, Sh=0.85, PF=1.38, Tr=26)
- htf_ema: FAIL (0/8, Sh=-0.72, PF=0.91, Tr=43) — 구조적 실패 확정

Bundle OOS (4h BTC Cycle 408): **5/5 PASS 유지** (이전 리포트 2026-07-08 유지)

코드 변경 사항:
- `tests/test_feed_boundary.py`: TestIndicatorBoundaryC408 클래스 3개 테스트 추가
- `tests/test_drawdown_monitor.py`: get_size_multiplier 복합 케이스 3개 추가
- `src/backtest/walk_forward.py`: DEFAULT_GRIDS["htf_ema"]={} + 구조적 한계 주석

테스트: 8642 → 8648 총계 (+6), 8619 → 8625 passed (+6)

---

## [2026-07-09] Cycle 407 — B(리스크) + D(ML) + F(리서치)

**[B(리스크)] CircuitBreaker 미커버 복합 케이스 3개** (+3)

tests/test_circuit_breaker.py (Cycle407 B):
1. `test_daily_drawdown_exact_boundary_with_atr_surge`: daily_dd == limit 경계값 + ATR surge 동시 → 낙폭 우선, triggered=True, size_multiplier=0.0, volatility_surge=False
2. `test_rapid_decline_does_not_set_is_triggered`: rapid_decline 감지 시 result["triggered"]=True이지만 cb.is_triggered=False (비영속적 트리거 동작 명시)
3. `test_consecutive_loss_cooldown_ignores_atr_surge`: 쿨다운 활성 + ATR surge 동시 → 쿨다운 우선, triggered=True, size_multiplier=0.0, volatility_surge=False

**[D(ML)] optimize_frama() 타입 검증 + optimize_narrow_range() 엣지케이스 3개** (+3+2=5 → 합계+5)

tests/test_phase_d.py (Cycle407 D):
4. `test_optimize_frama_avg_oos_sharpe_is_float`: avg_oos_sharpe isinstance float 타입 검증
5. `test_optimize_frama_oos_sharpe_std_non_negative`: oos_sharpe_std >= 0.0 (표준편차 비음수)
6. `TestOptimizeNarrowRange::test_optimize_narrow_range_single_window_no_crash`: n_windows=1 크래시 없음
7. `TestOptimizeNarrowRange::test_optimize_narrow_range_result_fields_present`: 핵심 필드 존재 검증
8. `TestOptimizeNarrowRange::test_optimize_narrow_range_oos_sharpe_std_non_negative`: oos_sharpe_std >= 0.0

**[F(리서치)] acceleration_band 1h BTC 구조적 한계 분석 + walk_forward.py 문서화**

acceleration_band 분석 (rank 15, Cycle407):
- BTC 1h: Sh=-0.94, PF=0.98(**<1.0=음의 엣지**), Trades=44, Consistency=1/8
- ETH 1h: Sh=-2.03, PF=0.57, Trades=13 (<15 min) — 신호 부족
- SOL 1h: Sh=-0.80, PF=1.00, Trades=11 (<15 min) — 신호 부족
- 핵심 원인: ATR band breakout은 TRENDING 환경에 적합, BTC 1h 47.3% RANGING → 허위 돌파 과다
- 파라미터화 가능 요소 없음 (period=20, margins 하드코딩)
- **결론**: acceleration_band 추가 탐색 금지 → walk_forward.py DEFAULT_GRIDS["acceleration_band"]={} 주석 추가

Paper Simulation (1h BTC Cycle 407): **PASS 1/19 유지** (변화 없음)
- roc_ma_cross: PASS (4/8, Sh=1.81, PF=2.02, Tr=14, MDD=3.4%)
- price_cluster: FAIL (2/8, Sh=1.06, PF=1.32, Tr=35)
- dema_cross: FAIL (2/8, Sh=0.85, PF=1.38, Tr=26)
- acceleration_band: FAIL (1/8, Sh=-0.94, PF=0.98, Tr=44) — 구조적 실패 확정

Bundle OOS (4h BTC Cycle 407): **5/5 PASS 유지** (SSL차단으로 이전 리포트 유지)

코드 변경 사항:
- `tests/test_circuit_breaker.py`: 복합 케이스 3개 추가 (47→ 총계 추가)
- `tests/test_phase_d.py`: optimize_frama 2개 + TestOptimizeNarrowRange 3개 추가
- `src/backtest/walk_forward.py`: DEFAULT_GRIDS["acceleration_band"]={} + 구조적 한계 주석
...
tests/test_drawdown_monitor.py (Cycle406 B):
1. `test_crisis_regime_tightens_daily_limit`: CRISIS 레짐 → HIGH_VOL과 동일 분기(_high_vol_daily_limit=2%) 적용 확인
2. `test_high_vol_regime_and_transition_cushion_compound`: HIGH_VOL(일일 한도 2%) + transition_cushion(저신뢰도→0.5x) 동시 활성 복합 검증
3. `test_regime_reset_reverts_daily_limit`: HIGH_VOL→TREND_UP 전환 후 daily 리셋 → 일일 한도 3% 원복

**[D(ML)] WalkForwardTrainer.select_features_pfi() 추가 경계 케이스 3개** (+3)

tests/test_ml_pipeline_edge_cases.py (TestSelectFeaturesPfi, Cycle406 D):
4. `test_select_features_pfi_top_k_zero_enforces_minimum`: top_k=0 → k=max(2,0)=2, 최소 2개 반환
5. `test_select_features_pfi_exactly_100_samples_normal_path`: n_samples=100 (경계값, <100 아님) → n_repeats=5 정상 경로
6. `test_select_features_pfi_top_k_equals_feature_count`: top_k=n_features=5 → 전체 5개 피처 반환

**[F(리서치)] narrow_range 1h BTC 구조적 한계 분석 + walk_forward.py 문서화**

narrow_range BTC 1h WFO 결과: Sh=-0.51, PF=0.97(<1.0), Trades=46, MDD=10.1%, 0/8 Consistency
- PF<1.0 → 손실이 이익을 초과 → 파라미터 조정으로 근본 해결 불가
- EMA slope filter(ema_slope_min_buy=0.0005): Trades=46→~20-25 감소 예상 → Trades<15 위험, PF 개선 불확실
- NR7(엄격): 거래 수 추가 감소 + 동일 구조 문제 → 근본 해결 안 됨
- 핵심 원인: 1h BTC 고빈도 노이즈 → NR breakout 지속성 부재 (4h/daily 타임프레임에 적합)
- **결론**: narrow_range 1h 탐색 보류 확정 → walk_forward.py DEFAULT_GRIDS["narrow_range"] 주석 추가
- dema_cross 4h Bundle OOS 후보 재검토: 4h 실 데이터 없음(SSL 차단) → 실험 불가

Paper Simulation (1h BTC Cycle 406): PASS 1/19 유지
- roc_ma_cross: PASS (4/8, Sh=1.81, PF=2.02, Tr=14, MDD=3.4%)
- price_cluster: FAIL (2/8, Sh=1.06, PF=1.32, Tr=35)
- dema_cross: FAIL (2/8, Sh=0.85, PF=1.38, Tr=26)
- narrow_range: FAIL (0/8, Sh=-0.51, PF=0.97, Tr=46) — 구조적 한계 확정

Bundle OOS (4h BTC Cycle 406): 5/5 PASS 유지 (SSL차단으로 이전 리포트 유지)

테스트: 8628 → 8634 총계 (+6), 8611 passed + 23 skipped = 8634

---

## [2026-07-08] Cycle 405 — A(품질) + C(데이터) + F(리서치)

**[A(품질)] BacktestEngine 극단 슬리피지/커미션 케이스 3개** (+3)

tests/test_backtest_engine.py (Cycle405 A):
1. `test_extreme_slippage_no_crash`: slippage=1.0(100%) 극단값 → 크래시 없음, total_return float 반환
2. `test_extreme_commission_no_crash`: commission=0.5(50%) 극단값 → 크래시 없음
3. `test_extreme_slippage_worsens_result`: slippage=1.0 → slippage=0 대비 total_return 같거나 낮음

**[C(데이터)] DataFeed 지표 엣지케이스 3개** (+3)

tests/test_feed_boundary.py (TestIndicatorEdgeCases, Cycle405 C):
4. `test_rsi14_nan_first_rows`: 30행 데이터 → rsi14 첫 14행 내 NaN 존재 (warm-up 필요)
5. `test_bb_upper_gte_bb_lower`: 50행 정상 데이터 → bb_upper >= bb_lower 항상 성립
6. `test_volume_zero_no_inf_no_crash`: volume=0 → 크래시 없음, vwap/volume_quote에 inf 없음

**[F(리서치)] lob_maker 구조적 한계 분석**

BTC 1h paper_sim: rank5, Sh=-0.04, Trades=75, MDD=17%, 0/8 Consistency
- 근본 원인: LOB 데이터(bid_vol/ask_vol) 없음 → OFI proxy = (close-open)/(high-low) 사용
  캔들 body/range 비율 — 실제 order flow 불균형 측정 불가
- VPIN도 OHLCV 추정 → fallback=0.5(불확실)
- RSI 필터 역방향: rsi<22 시 BUY 차단 (과매도=추세 역행 방지 의도)
- **결론**: LOB 인프라 없이 파라미터 조정으로 PASS 불가 → 탐색 완전 보류
- walk_forward.py DEFAULT_GRIDS["lob_maker"] 추가 (구조적 한계 주석 포함)

Paper Simulation (1h BTC Cycle 405): PASS 1/19 유지
- roc_ma_cross: PASS (4/8, Sh=1.81, PF=2.02, Tr=14, MDD=3.4%)
- price_cluster: FAIL (2/8, Sh=1.06, PF=1.32, Tr=35)
- dema_cross: FAIL (2/8, Sh=0.85, PF=1.38, Tr=26)
- lob_maker: FAIL (0/8, Sh=-0.04, Tr=75, MDD=17%) — 구조적 한계 확정

Bundle OOS (4h BTC Cycle 405): 5/5 PASS 유지 (SSL차단으로 이전 리포트 유지)

테스트: 8622 → 8628 총계 (+6), 8605 passed + 23 skipped = 8628

---

## [2026-07-08] Cycle 404 — D(ML) + E(실행) + F(리서치)

**[D(ML)] WalkForwardTrainer.select_features_pfi() 엣지케이스 3개** (+3)

tests/test_ml_pipeline_edge_cases.py (TestSelectFeaturesPfi):
1. `test_select_features_pfi_small_xtrain_no_crash`: X_train 20행 (<100) → n_repeats=10 경로, 크래시 없이 피처 리스트 반환
2. `test_select_features_pfi_top_k_exceeds_feature_count`: top_k=20, 피처=3 → k=max(2,min(20,3))=3, 전체 3개 반환
3. `test_select_features_pfi_top_k_one_enforces_minimum`: top_k=1, 피처=3 → k=max(2,1)=2, 최소 2개 강제

**[E(실행)] PaperConnector 포지션 state 3개** (+3)

tests/test_exchange.py (TestPaperConnectorCycle404):
4. `test_open_positions_after_buy`: buy 후 open_positions[BTC/USDT] > 0 검증
5. `test_open_positions_cleared_after_sell`: buy→sell 후 open_positions qty=0 검증
6. `test_timeout_order_does_not_modify_position`: timeout_prob=1.0 → 주문 취소, 잔고/포지션 불변

**[F(리서치)] engulfing_zone / volatility_cluster Bundle OOS 후보 검토**

BTC 1h 전체 데이터 분석:
- engulfing_zone: Sh=-0.79, PF=0.93, Trades=657, MDD=52.5% → 구조적 실패 (너무 많은 거래, 음수 Sharpe)
- BTC 4h: engulfing_zone Sh=-2.74 (더 나쁨) → Bundle OOS 후보 완전 부적합
- volatility_cluster BTC 4h: Sh=0.32, PF=1.06, MDD=12.2% → PASS 기준 미달 (Sh<1.0, PF<1.5)
**결론**: 현재 Bundle OOS 5번째 이외의 신규 후보 없음. 기존 5/5 PASS 전략 강화 집중.

Paper Simulation (1h BTC Cycle 404): PASS 1/19 유지
- roc_ma_cross: PASS (4/8, Sh=1.81, PF=2.02, Tr=14, MDD=3.4%)
- price_cluster: FAIL (2/8, Sh=1.06, PF=1.32, Tr=35)
- dema_cross: FAIL (2/8, Sh=0.85, PF=1.38, Tr=26)

Bundle OOS (4h BTC Cycle 404): 5/5 PASS 유지 (동일)
테스트 수: 8616 → 8622 (+6)

---

## [2026-07-07] Cycle 403 — C(데이터) + B(리스크) + F(리서치)

**[C(데이터)] _add_indicators() 신규 컬럼 엣지케이스 3개 추가** (+3)

tests/test_feed_boundary.py (TestAddIndicatorsNewColumnsEdge):
1. `test_ema200_column_present_and_last_value_finite`: 50행 → ema200 컬럼 생성 및 마지막 행 유한값 검증
2. `test_ema20_slope_near_zero_for_constant_close`: 상수 close → ema20_slope ≈ 0 검증
3. `test_return_5_nan_first_rows_finite_after_five`: pct_change(5) → 첫 5행 NaN, 이후 유한값 검증

**[B(리스크)] DrawdownMonitor reset_daily() 복합 케이스 3개 + KellySizer compute_dynamic() 3개** (+6)

tests/test_drawdown_monitor.py:
4. `test_reset_daily_not_halted_updates_daily_start`: 미정지 상태 → daily_start만 갱신, 상태 변화 없음
5. `test_reset_daily_halt_level_not_cleared`: HALT 레벨 → reset_daily()로 해제 안 됨 (WARNING만 해제)
6. `test_reset_daily_force_liquidate_not_cleared`: FORCE_LIQUIDATE → reset_daily()로 해제 안 됨

tests/test_kelly_sizer_regime_edge_cases.py (TestComputeDynamicBoundary):
7. `test_compute_dynamic_no_history_returns_min_fraction_capital`: 빈 이력 → min_fraction*capital 반환
8. `test_compute_dynamic_below_min_trades_uses_fallback`: min_trades 미만 → fallback(min_fraction*capital)
9. `test_compute_dynamic_sufficient_history_returns_positive`: 충분한 수익 이력 → 양수 사이즈 반환

**[F(리서치)] positional_scaling 구조 분석 + walk_forward.py DEFAULT_GRIDS 추가**

Paper Simulation (1h BTC): PASS 1/19 유지
- roc_ma_cross: PASS (4/8, Sh=1.81, PF=2.02, Tr=14, MDD=3.4%)
- positional_scaling: FAIL (1/8, Sh=-0.38, PF=1.09, Tr=34, SharpeStd=2.82)

Bundle OOS (4h BTC): 5/5 PASS 유지

positional_scaling 구조 분석:
- 구조적 문제 1: triple EMA alignment(20/50/100) — RANGING 47.3%에서 정렬 빈도 낮음
- 구조적 문제 2: pullback/rally 조건 동일 (deviation = c/e20-1 ±threshold)
- 구조적 문제 3: pullback_atr_mult=0.3 하드코딩 — ATR*0.3/e20 ≈ 0.45% 좁은 범위
- 결론: Sh=-0.38(음수) + SharpeStd=2.82(고변동) → 구조적 신호 품질 문제. 탐색 보류.
- walk_forward.py DEFAULT_GRIDS["positional_scaling"] 추가 (파라미터화 후 탐색 방향 문서화)

**[요약]**
- 테스트 8607 → 8616 (+9)
- walk_forward.py DEFAULT_GRIDS["positional_scaling"] 추가 (구조 분석 기록)
- 시뮬: 1h PASS 1/19, Bundle OOS 5/5

---

## [2026-07-07] Cycle 402 — E(실행) + A(품질) + F(리서치)

**[E(실행)] PaperConnector 미커버 케이스 3개 추가** (+3)

tests/test_exchange.py (TestPaperConnectorCycle402):
1. `test_create_order_invalid_action_raises_error`: "hold" 액션 → PaperTrader error 상태 → ValueError("Order error") 발생
2. `test_create_order_partial_fill_info_flag`: partial_fill_prob=1.0 → info["is_partial"]=True, filled < amount
3. `test_create_order_filled_id_has_string_prefix`: 정상 체결 시 id가 "paper_order_" 접두사 포함

**[A(품질)] BacktestEngine.apply_wfe() 미커버 케이스 + 버그픽스 4개 추가** (+4)

tests/test_backtest_engine.py:
4. `test_apply_wfe_positive_is_sets_ratio`: IS=2.0, OOS=1.0 → wfe=0.5 저장
5. `test_apply_wfe_negative_is_large_oos_gives_partial`: IS=-2.0, OOS=2.0 → wfe=0.5 (역방향 레짐, 강한 OOS)
6. `test_apply_wfe_negative_is_small_oos_gives_zero`: IS=-2.0, OOS=1.0 → wfe=0.0 (강한 역방향)
7. `test_summary_negative_wfe_shown_not_na`: 음수 wfe → summary()에서 실제 값 표시 (N/A 아님)

src/backtest/engine.py 버그픽스:
- `BacktestResult.summary()`: `wfe > 0` → `wfe != 0.0` 조건 변경
- 음수 WFE(IS과최적화→OOS역방향)가 "N/A"로 숨겨지던 버그 수정
- wfe==0.0만 "N/A"(미계산), 음수는 실제 값 표시

**[F(리서치)] 시뮬레이션 결과 분석 — 탐색 완료 선언**

Paper Simulation (1h BTC): PASS 1/19 유지
- roc_ma_cross: PASS (4/8, Sh=1.81, PF=2.02, Tr=14, MDD=3.4%)
- price_cluster: FAIL (2/8, Sh=1.06, PF=1.32) — 구조적 ceiling 확정
- dema_cross: FAIL (2/8, Sh=0.85, PF=1.38) — 모든 방향 소진
- frama: FAIL (0/8, Sh=0.44) — 구조적 한계, 탐색 종료

Bundle OOS (4h BTC): 5/5 PASS 유지

연구 결론:
- 1h BTC 주요 전략(price_cluster, dema_cross, frama) 최적화 파라미터 공간 소진
- roc_ma_cross Consistency 개선: FAIL folds는 저거래량 레짐 구조적 의존성 → 파라미터로 해결 불가
- Bundle OOS 4h 5/5 안정적 → 다음 사이클에서 새 전략 후보 발굴 또는 Bundle 확장 검토

**[요약]**
- 테스트 8600 → 8607 (+7)
- engine.py summary() 음수 WFE 표시 버그픽스
- 시뮬: 1h PASS 1/19, Bundle OOS 5/5

---

## [2026-07-07] Cycle 401 — B(리스크) + D(ML) + F(리서치)

**[B(리스크)] DrawdownMonitor set_sharpe_decay 복합 조합 6개 추가** (+6)

tests/test_drawdown_monitor.py:
1. `test_sharpe_decay_and_atr_elevated_compound`: ATR elevated + Sharpe decay → get_size_multiplier()=0.5
2. `test_sharpe_decay_negative_recent_sharpe_is_decayed`: recent_sharpe<0 → decay 발동
3. `test_sharpe_decay_recovery_resets_multiplier`: 회복 후 multiplier=1.0 복원
4. `test_sharpe_decay_zero_recent_sharpe`: recent_sharpe=0.0 → decay 발동
5. `test_sharpe_decay_custom_threshold_boundary`: ratio==threshold → < 조건 불성립 → 1.0
6. `test_sharpe_decay_and_mdd_warn_compound`: MDD WARN + Sharpe decay 동시 → min(0.5,1.0,1.0,0.5)=0.5

**[D(ML)] optimize_frama 파라미터 검증 3개 추가** (+3)

tests/test_phase_d.py (TestOptimizeFrama):
7. `test_optimize_frama_weak_rsi_key_in_best_params`: best_params에 weak_rsi_buy_max 포함 확인 (1200행 보장)
8. `test_optimize_frama_grid_combos_count`: DEFAULT_GRIDS["frama"] 27 combos (3*3*3=period×rsi_period×weak_rsi)
9. `test_optimize_frama_no_atr_period_in_grid`: atr_period DEAD PARAM → 그리드 미포함 검증

**[F(리서치)] frama 0/8 Consistency 근본 원인 코드 분석 완료**

- frama.py 신호 구조 분석: strong_signal(gap>=1%→RSI<85) vs weak_signal(gap<1%→RSI<weak_rsi_buy_max)
- RANGING(47.3% BTC 1h): gap<1% 지배 → weak 신호 경로가 주경로, RSI 중립(40-60) 구간 차단
- weak_rsi_buy_max=40/50/60 전부: RSI 40-60 중립 구간 차단으로 RANGING에서 구조적 신호 차단
- atr_contracting DEAD PARAM 재확인: BUY/SELL 조건 미사용(로그 전용) → atr_period 탐색 무의미
- 결론: frama WFO 그리드 [40,50,60] 유지 (자동 선택), frama 추가 탐색 종료
- walk_forward.py: Cycle401 F(리서치) 분석 주석 추가

**[요약]**
- 테스트 8568 → 8577 (+9)
- roc_ma_cross 유일 PASS 유지 (Sh=1.81, PF=2.02, Consist=4/8)
- Bundle OOS: 5/5 PASS 유지 (cmf, order_flow_imbalance_v2, supertrend_multi, vwap_cross, value_area)
- frama 탐색 완전 종료 (RANGING 구조적 한계 확정)

**[Paper Simulation (WF 1h)] 2026-07-07T05:20:43 BTC/USDT**
- roc_ma_cross: **PASS** (Sh=1.81, PF=2.02, Tr=14, Consist=4/8)
- price_cluster: FAIL (Sh=1.06, PF=1.32, Tr=35, Consist=2/8)
- dema_cross: FAIL (Sh=0.85, PF=1.38, Tr=26, Consist=2/8)
- frama: FAIL (Sh=0.44, PF=1.11, Tr=65, Consist=0/8) — 구조적 한계 확정
- 1/19 PASS

**[Bundle OOS (4h)] 2026-07-07T05:18:30 BTC/USDT**
- 5/5 PASS (cmf, order_flow_imbalance_v2, supertrend_multi, vwap_cross, value_area)
...
BacktestEngine (test_backtest_engine.py):
1. `AlternatingStrategy` 헬퍼 클래스 추가 (BUY/SELL 교번 전략)
2. `test_position_flip_buy_sell_generates_multiple_trades`: BUY→SELL→BUY 방향 전환 시 2회+ 거래 생성 검증
3. `test_engine_reuse_identical_results`: 동일 엔진 재사용 시 잔고 초기화 → 동일 결과 보장
4. `test_engine_two_strategies_result_names`: 두 전략 실행 시 각 결과에 전략명 올바르게 기록

optimize_frama (test_phase_d.py):
5. `TestOptimizeFrama.test_optimize_frama_returns_wf_result`: 기본 호출 → WalkForwardResult 반환
6. `TestOptimizeFrama.test_optimize_frama_single_window_no_crash`: n_windows=1 단일 윈도우 크래시 없음
7. `TestOptimizeFrama.test_optimize_frama_result_fields_present`: avg_oos_sharpe, best_params 등 핵심 필드 존재

**[C(데이터)] DataFeed duplicate timestamp 제거 + 동일 심볼 캐시 테스트 9개 추가** (+9)

TestToDataframeDuplicateTimestamps (test_feed_boundary.py):
8. `test_duplicate_timestamps_removed_keep_last`: 동일 타임스탬프 2개 → 마지막 값 유지, 1행
9. `test_three_duplicate_timestamps_keep_last`: 3개 중복 → 1행 유지
10. `test_partial_duplicates_unique_retained`: 일부 중복 + 고유 → 2행 정상 처리

TestSameSymbolMultipleRequests (test_feed_boundary.py):
11. `test_same_symbol_second_request_hits_cache`: 동일 심볼 2번째 요청 → 캐시 히트
12. `test_different_limit_is_different_cache_key`: limit 다름 → 별도 캐시 키 → 각각 미스
13. `test_different_symbol_is_different_cache_key`: 심볼 다름 → 별도 캐시 → 2번 미스

**[F(리서치)] frama weak_rsi_buy_max 탐색 분석**
- Cycle400 paper_sim 결과: frama Sh=0.44, Trades=65, PF=1.11, 0/8 Consistency (동일 — Cycle399 확정값 유지)
- WFO 그리드 [40,50,60] 탐색 중: weak_rsi_buy_max=50 유지 확정
- 0/8 Consistency는 frama 구조적 한계 — 파라미터 탐색으로 해소 불가
- weak_rsi_sell_min=50 현재 설정 유지: SELL 신호 허용 범위 확대(RSI>50), PF 소폭 하락 감수
- **다음 탐색 방향**: WFO best params [40,50,60] 중 fold별 선택 분포 분석 또는 다른 전략 집중
- Bundle OOS: 합성 데이터 fallback → 기존 5/5 PASS 유지 (실거래소 SSL 차단)

**[요약]**
- 테스트 8556 → 8568 (+12)
- roc_ma_cross 유일 PASS 유지 (Sh=1.81, 4/8)
- 기존 테스트 모두 통과 (8568 passed, 23 skipped)
