...
## [2026-04-20] SHAP 피처 선택 + 크립토 ML 피처 엔지니어링 리서치

### 1. SHAP 활용 크립토 ML 피처 선택 실제 사례

**사례 A: 멀티모달 BTC 예측 (arxiv 2410.14475, 2024)**
- 43개 거래소+온체인 피처 + 요일 더미 변수로 시작, SHAP으로 중요도 분석
- 살아남은 피처 카테고리: 거래 미시구조(가격/거래량), 온체인 메트릭, 규제/매크로 뉴스 태그, 온라인 감성 점수
- 중요도는 시장 구간별로 크게 변함: 2018년엔 Hash Difficulty가 1위(평균 예측의 ~20%), 횡보 구간에서는 Google Trends가 1위(~23%)
- **교훈**: 단일 피처셋으로 모든 레짐을 커버 불가. 레짐별 피처 가중치 동적 재계산 필요.
- 출처: https://arxiv.org/pdf/2410.14475

**사례 B: SHAP 기반 반복 피처 제거 (arxiv 2410.06815, 2024)**
- `shap-select` 라이브러리 — SHAP 값 1회 계산 후 최하위 피처부터 선형/로지스틱 회귀로 반복 제거
- HISEL, RFE, 기존 shap-selection 대비 모델 성능 우수 + 계산 비용 낮음
- 고차원 데이터(피처 수 >> 샘플 수)에서 특히 효과적, 과적합 억제 확인
- **교훈**: 전체 모델 재훈련 없이 SHAP 1회 실행 → 반복 제거 루프로 최적 피처셋 도달 가능. ~1000샘플에 적합.
- 출처: https://arxiv.org/html/2410.06815v1

---

### 2. 소규모 데이터(~1000샘플)에서 피처 선택 방법 비교

| 방법 | 장점 | 단점 | ~1000샘플 권고 |
|------|------|------|----------------|
| Permutation Importance | 예측 성능 기반, 해석 직관적 | 상관 피처 간 중요도 분산 | 적합 (빠름) |
| SHAP | 전역+국소 해석 가능, 상호작용 캡처 | 계산 비용 높음(모든 조합) | TreeSHAP 사용 시 적합 |
| RFE | 모델 성능 직접 최적화, 조합 효과 반영 | 재훈련 반복 → 느림 | 소규모에선 허용 가능 |

- 핵심 결론: 3가지 방법이 동시에 지목하는 피처 = "backbone" 피처, 가장 신뢰도 높음
- Permutation Importance와 SHAP이 일치하지 않을 때: SHAP은 모델 내 기여도, PFI는 성능 변화 기반으로 측정 방식이 다르기 때문. 두 결과를 교집합으로 사용 권고.
- 출처: https://arxiv.org/html/2410.06815v1, https://scikit-learn.org/stable/modules/permutation_importance.html, https://link.springer.com/article/10.1186/s40537-024-00905-w

---

### 3. 크립토 트레이딩봇에서 가장 유용한 피처 TOP 5

실전 사례(FreqAI 생산 환경) + 논문 종합:

1. **거래량 파생 지표** (MFI, OBV, 거래량 이동평균 비율) — 가격 이동의 선행 신호, 미시구조에서 일관되게 상위 랭크
2. **변동성 메트릭** (ATR 14/21, 실현 변동성 20일) — 레짐 감지 + 포지션 사이징 동시에 활용 가능한 이중 역할 피처
3. **모멘텀 지표** (RSI, EMA 크로스 비율, ROC) — 단기 추세 포착, 소규모 샘플에서도 안정적
4. **온체인 메트릭** (Hash Rate, 거래소 유입/유출, 고래 주소 활성도) — 2024~2025 SHAP 분석에서 중요도 상위권, 단 레짐에 따라 변동 큼
5. **감성/관심도 지표** (Google Trends, FinBERT 뉴스 점수) — 횡보 레짐에서 가장 큰 예측력, 추세 레짐에서는 가격 지표에 밀림

**주의**: 중요도 순위는 시장 레짐에 따라 역전됨. 단일 피처셋 고정 금지.
- 출처: https://www.freqtrade.io/en/stable/freqai-feature-engineering/, https://arxiv.org/pdf/2410.14475

---

### 4. ExtraTrees vs Random Forest 크립토 데이터 실전 비교

| 항목 | ExtraTrees | Random Forest |
|------|-----------|---------------|
| 분할 방식 | 완전 무작위 (random cut) | 탐욕적 최적 분할 |
| 훈련 속도 | ~3배 빠름 | 느림 |
| 편향 | 높음 (무작위 분할로 구조 단순화) | 낮음 |
| 분산 | 낮음 (트리 간 상관 감소) | 높음 |
| 소규모 데이터 정확도 | 낮음 (과적합 억제 대신 편향 증가) | 높음 |
| 고차원 피처 | 유리 (불필요 피처 포함해도 분산 낮춤) | 불리 |
| 크립토 노이즈 환경 | 분산 낮아 안정적이나 신호 약화 위험 | 더 날카로운 신호 포착 가능 |

- 원본 논문(Geurts 2006): 불필요 피처 존재 시 ExtraTrees 편향이 RF보다 큼. 피처 선택 후 불필요 피처 제거 시 편향 수렴.
- **~1000샘플 크립토 데이터 권고**: RF 우선. 단, 훈련 시간이 병목이거나 피처 수가 샘플 수를 초과하면 ExtraTrees 전환.
- 출처: https://jasonphang.com/files/extratrees.pdf, https://www.geeksforgeeks.org/machine-learning/randomforestclassifier-vs-extratreesclassifier-in-scikit-learn/

---

### 우리 프로젝트 핵심 교훈 3개 (2026-04-20 SHAP/피처 엔지니어링)

1. **SHAP 1회 계산 → 반복 제거(shap-select 방식) 도입**: 현재 ML 모델이 고정 피처셋 사용 중이면, SHAP 값 1회 계산 후 최하위 피처부터 제거하는 반복 루프를 훈련 파이프라인에 추가. ~1000샘플에서도 계산 부담 낮고 과적합 억제 효과 확인됨. `src/` ML 관련 모듈에 `shap-select` 패턴 적용 우선순위.
