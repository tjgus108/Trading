...
- BUY: quality_score > 1.0 AND mom20 > 0
- SELL: quality_score < -0.5 AND mom20 < 0
...
class MomentumQualityStrategy(BaseStrategy):
    ...
    def generate(self, df: pd.DataFrame) -> Signal:
        ...
        # NaN 처리
        if pd.isna(mom20_val) or pd.isna(quality_score_val):
        ...
        context = (
            ...
            f"curr_close={curr_close:.4f}"
        )

        if quality_score_val > 1.0 and mom20_val > 0:
            ...
            return Signal(
                ...
                reasoning=f"고품질 상승 모멘텀: {context}",
                invalidation="quality_score <= 1.0 또는 mom20 <= 0",
                bull_case=f"일관되고 가속되는 상승 모멘텀 (score={quality_score_val:.3f})",
                bear_case="모멘텀 품질 하락 시 빠른 청산",
            )

        if quality_score_val < -0.5 and mom20_val < 0:
            ...
            return Signal(
                ...
                strategy=self.name,
                entry_price=curr_close,
                reasoning=f"고품질 하락 모멘텀: {context}",
                invalidation="quality_score >= -0.5 또는 mom20 >= 0",
                bull_case="모멘텀 반전 시 숏 포지션 청산",
                bear_case=f"일관되고 가속되는 하락 모멘텀 (score={quality_score_val:.3f})",
            )
