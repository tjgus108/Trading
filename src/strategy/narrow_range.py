...
class NarrowRangeStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            ema_slope_min_buy: BUY 신호 최소 EMA slope (음수 slope=하락추세 차단).
            ema_slope_max_sell: SELL 신호 최대 EMA slope (양수 slope=상승추세 차단).
        """
    ...
    def generate(self, df: pd.DataFrame) -> Signal:
        ...
        if close_curr > high_nr:
            if ema_slope is not None and ema_slope < self.ema_slope_min_buy:
                return self._hold(
                    df,
                    f"ema_slope_min_buy 미충족: slope={ema_slope:.6f} < {self.ema_slope_min_buy}",
                )
            return Signal(
                action=Action.BUY,
                confidence=conf,
        ...
        if close_curr < low_nr:
            if ema_slope is not None and ema_slope > self.ema_slope_max_sell:
                return self._hold(
                    df,
                    f"ema_slope_max_sell 미충족: slope={ema_slope:.6f} > {self.ema_slope_max_sell}",
                )
            return Signal(
                action=Action.SELL,
                confidence=conf,
