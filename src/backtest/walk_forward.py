...
DEFAULT_GRIDS: Dict[str, dict] = {
    ...
    # Cycle357 F(리서치): filter=False 실험 → Sharpe=0.87로 filter=True와 동일 → [False, True] 그리드 추가
    #   결론: vol_regime_filter True/False 모두 동일 Sharpe(0.87) → 필터 자체가 bottleneck 아님
    #   bottleneck: PF 1.20 < 1.5. vol_regime_filter 탐색은 계속 유지 (False가 더 빠를 수 있음)
    "price_cluster": {
