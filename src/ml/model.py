...
class MLSignalGenerator:
    ...
    def __init__(
        ...
        self._feature_names: List[str] = []  # 학습 시 사용된 피처 컬럼 (예측 시 필터링)
        self._trained_regime: Optional[str] = None
    ...
    def predict(self, df: pd.DataFrame) -> MLPrediction:
        ...
        try:
            ...
            # 학습 시 사용된 피처만 선택 (regime-aware 모델 피처 불일치 방지)
            if self._feature_names:
                missing = set(self._feature_names) - set(feat_df.columns)
                if missing:
                    logger.warning("ML predict: %d 피처 누락 %s", len(missing), list(missing)[:3])
