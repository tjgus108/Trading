...
class TestFeatureCountDiff:
    ...
    def test_no_fr_oi_base_14_features(self):
        """FR/OI 없이 base 피처만 생성 (20 base + vpin_50 = 21개)."""
        fb = FeatureBuilder()
        df = _make_ohlcv(100)
        X, y = fb.build(df)
        assert len(X.columns) == 21  # base 20 + vpin_50
    ...
    def test_fr_only_15_features(self):
        """funding_rate만 있으면 base 21 + delta_fr = 22."""
        fb = FeatureBuilder()
        df = _make_ohlcv(100)
        df["funding_rate"] = np.random.uniform(-0.001, 0.001, len(df))
        X, y = fb.build(df)
        assert len(X.columns) == 22  # base 20 + vpin_50 + delta_fr
        assert "delta_fr" in X.columns
        assert "fr_oi_interaction" not in X.columns

    def test_fr_and_oi_16_features(self):
        """funding_rate + open_interest 둘 다 있으면 base 21 + 2 = 23."""
        fb = FeatureBuilder()
        df = _add_fr_oi(_make_ohlcv(100))
        X, y = fb.build(df)
        assert len(X.columns) == 23  # base 20 + vpin_50 + delta_fr + fr_oi_interaction
        assert "delta_fr" in X.columns
        assert "fr_oi_interaction" in X.columns

    def test_btc_close_plus_fr_oi_17_features(self):
        """btc_close + funding_rate + open_interest → 24 피처."""
        fb = FeatureBuilder()
        df = _add_fr_oi(_make_ohlcv(100))
        df["btc_close"] = df["close"] * 1.1  # dummy BTC close
        X, y = fb.build(df)
        assert len(X.columns) == 24  # base 20 + vpin_50 + btc_close_lag1 + delta_fr + fr_oi
...
class TestTrainerWithFrOi:
    ...
    def test_train_with_fr_oi_features(self):
        ...
        assert result.n_features == 23  # base 20 + vpin_50 + delta_fr + fr_oi_interaction

    def test_train_without_fr_oi_features(self):
        """FR/OI 없는 데이터로 학습 시 base 피처만 (vpin_50 포함 21개)."""
        trainer = WalkForwardTrainer(
            symbol="TEST/USDT", n_estimators=10, max_depth=3,
        )
        df = _make_ohlcv(300)
        ...
        assert result.n_features == 21  # base 20 + vpin_50
