...
PAPER_SIM_STRATEGY_PARAMS: Dict[str, dict] = {
    ...
    # Cycle298 B: rvol_buy_sell 1.2→1.1 (일부 윈도우 11/14 trades FAIL → 신호 빈도 추가 완화)
    "relative_volume": {"rvol_buy_sell": 1.1},
    # Cycle297 F: bull_only=True 역효과 (Sharpe 1.82→1.60, trades 22→19) → 제거
    "momentum_quality": {"quality_score_buy_threshold": 0.8, "consistency_buy_threshold": 0.3},
    # Cycle298 C: bounce_pct 0.015→0.02 (avg_trades=11 < 15 기준 → threshold 확대로 신호 빈도↑)
    # 결과: trades 11→12 (소폭 개선), 유지
    "price_cluster": {"bounce_pct": 0.02},
    # Cycle298 F: buy_thresh=0.22 시도 → consistency 3/8→2/8 퇴보 → 제거 (기본값 0.25 복원)
}
